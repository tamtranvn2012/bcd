...
function wppb_heading_handler($page, $item_title, $id, $filterArray, $fieldName){
	...
	$filterArray['headerCustomField'] = apply_filters('wppb_'.$page.'_header_custom_field_'.$id, $filterArray['headerCustomField']);
	$filterArray['headerCustomField'];
...
function wppb_input_handler($page, $item_title, $id, $item_desc, $item_required, $item_id, $item_type, $currentUser, $filterArray, $extraFieldsErrorHolder, $postData, $item_options, $error, $fieldName){
	...
	if ($page == 'back_end'){
		$filterArray['inputCustomField'] = '
			<table class="form-table">
				<tr>
					<th><label for="'.$item_title.'">'.$item_title.'</label></th>
					<td>
						<input class="text-input" size="45" name="'.$item_id.$id.'" type="text" id="'.$item_type.'" value="'.$inputValue.'"';
			...
			$filterArray['inputCustomField'] = apply_filters('wppb_'.$page.'_input_custom_field_'.$id, $filterArray['inputCustomField']);		
	}else{
		$errorVar = '';
		$errorMark = '';
		...
		if ($page == 'register'){
			if (isset($postData[$item_id.$id]))
				$inputValue = $postData[$item_id.$id];
			else $inputValue = '';
		}
		$filterArray['inputCustomField'] = '
			<p class="form-input'.$id.$errorVar.'">
				<label for="'.$item_title.'">'.$item_title.$errorMark.'</label>
				<input class="text-input" name="'.$item_id.$id.'" type="text" id="'.$item_title.'" value="'.$inputValue.'"';
					if (($item_options != '') || ($item_options != NULL))
						$filterArray['inputCustomField'] .= 'maxlength="'.$item_options.'"';
...
function wppb_hidden_input_handler($page, $item_title, $id, $item_desc, $item_required, $item_id, $item_type, $currentUser, $filterArray, $extraFieldsErrorHolder, $postData, $item_options, $error, $fieldName){

	if ($page == 'back_end'){
		$filterArray['hiddenInputCustomField'] = '
			<!--form-input'.$id.'-->
				<label for="'.$item_title.'"><!--'.$item_title.'--></label>
				<input class="text-input" name="'.$item_id.$id.'" type="hidden" id="'.$item_title.'" value="'.$item_options.'"/>
				<span class="wppb-description-delimiter"><!--'.$item_desc.'--></span>					
			<!-- .form-input'.$id.' -->';
		
		$filterArray['hiddenInputCustomField'] = apply_filters('wppb_'.$page.'_hidden_input_custom_field_'.$id, $filterArray['hiddenInputCustomField']);		
	}else{
		$filterArray['hiddenInputCustomField'] = '
...
function wppb_checkbox_handler($page, $item_title, $id, $item_desc, $item_required, $item_id, $item_type, $currentUser, $filterArray, $extraFieldsErrorHolder, $postData, $item_options, $error, $fieldName){

	$userData = get_user_meta($currentUser, $fieldName, true);
	$userDataArray = explode(',', $userData);
	$checkBoxValue = $item_options;
	...
	if ($page == 'back_end'){
		...
		$filterArray['checkboxCustomField'] = apply_filters('wppb_'.$page.'_checkbox_custom_field_'.$id, $filterArray['checkboxCustomField']);		
	}else{
...
function wppb_radio_handler($page, $item_title, $id, $item_desc, $item_required, $item_id, $item_type, $currentUser, $filterArray, $extraFieldsErrorHolder, $postData, $item_options, $error, $fieldName){
	...
	if ($page == 'back_end'){
			...
			<table class="form-table">
				<tr>
					...
					foreach($radioValue as $thisValue){
							$filterArray['radioCustomField'] .= '<input value="'.$thisValue.'" name="'.$item_id.$id.'" type="radio"'; 
							if ((strpos($userData, $thisValue)) !== FALSE) 
		...
		$filterArray['radioCustomField'] = apply_filters('wppb_'.$page.'_radio_custom_field_'.$id, $filterArray['radioCustomField']);		
	}else{
			...
			<p class="form-radio'.$id.$errorVar.'">
				...
				<span class="wppb-description-delimiter">';
					foreach($radioValue as $thisValue){
						...
						if ($page == 'register'){
							$selValue = '';
							if (isset($_POST[$item_id.$id]))
								$selValue = $_POST[$item_id.$id];
							if ( $error && ( $selValue == $thisValue ) )
								$filterArray['radioCustomField'] .=  ' checked';
						}elseif ((strpos($userData, $thisValue)) !== FALSE) 
							$filterArray['radioCustomField'] .= ' checked';
...
function wppb_select_handler($page, $item_title, $id, $item_desc, $item_required, $item_id, $item_type, $currentUser, $filterArray, $extraFieldsErrorHolder, $postData, $item_options, $error, $fieldName){
	...
	if ($page == 'back_end'){
		$filterArray['selectCustomField'] = '
			<table class="form-table">
				<tr>
					<th><label for="'.$item_title.'">'.$item_title.'</label></th>
					<td>
					<select name="'.$item_id.$id.'" id="display_name">';
		...
		$filterArray['selectCustomField'] = apply_filters('wppb_'.$page.'_select_custom_field_'.$id, $filterArray['selectCustomField']);		
	}else{
		...
		$filterArray['selectCustomField'] = '
			<p class="form-select'.$id.$errorVar.'">
				<label for="'.$item_title.'">'.$item_title.$errorMark.'</label>
				<select name="'.$item_id.$id.'" id="'.$item_title.'">';
				foreach($selectValue as $thisValue){
					$filterArray['selectCustomField'] .= '<option value="'.$thisValue.'"'; 
					if ($page == 'register'){
						if ( $error && ( $_POST[$item_id.$id] == $thisValue ) ) 
							$filterArray['selectCustomField'] .= ' selected ';
					}elseif ((strpos($userData, $thisValue)) !== FALSE) 
						$filterArray['selectCustomField'] .= ' selected';
					$filterArray['selectCustomField'] .= '>'.$thisValue.'</option>';
...
function wppb_country_select_handler($page, $item_title, $id, $item_desc, $item_required, $item_id, $item_type, $currentUser, $filterArray, $extraFieldsErrorHolder, $postData, $item_options, $error, $fieldName){
	...
	if ($page == 'back_end'){
		...
		$filterArray['countrySelectCustomField'] = apply_filters('wppb_'.$page.'_country_select_custom_field_'.$id, $filterArray['countrySelectCustomField']);		
	}else{
...
/* the function to handle the COUNTRY SELECT custom field */
function wppb_timezone_select_handler($page, $item_title, $id, $item_desc, $item_required, $item_id, $item_type, $currentUser, $filterArray, $extraFieldsErrorHolder, $postData, $item_options, $error, $fieldName){
	...
	if ($page == 'back_end'){
		...
		$filterArray['timezoneSelectCustomField'] = apply_filters('wppb_'.$page.'_timezone_select_custom_field_'.$id, $filterArray['timezoneSelectCustomField']);		
	}else{
...
function wppb_datepicker_handler($page, $item_title, $id, $item_desc, $item_required, $item_id, $item_type, $currentUser, $filterArray, $extraFieldsErrorHolder, $postData, $item_options, $error, $fieldName){
	...
	if ($page == 'back_end'){
		$filterArray['datepickerCustomField'] = '
			<table class="form-table">
				<tr>
					<th><label for="'.$item_title.'">'.$item_title.'</label></th>
					<td>
						<input size="45" name="'.$item_id.$id.'" type="text" class="wppb_datepicker" value="'.$userData.'"/>
						<span class="description">'.$item_desc.'</span>
					</td>
				</tr>
			</table>';
		
		$filterArray['datepickerCustomField'] = apply_filters('wppb_'.$page.'_datepicker_custom_field_'.$id, $filterArray['datepickerCustomField']);		
	}else{
		...
		if ($page == 'register'){
			$userData = '';
			if (isset($_POST[$item_id.$id]))
				$userData = $_POST[$item_id.$id];
		}
		
		$filterArray['datepickerCustomField'] = '
			<p class="form-input'.$id.$errorVar.'">
				<label for="'.$item_title.'">'.$item_title.$errorMark.'</label>
				<input name="'.$item_id.$id.'" type="text" class="wppb_datepicker" value="'.$userData.'" />
				<span class="wppb-description-delimiter">'.$item_desc.'</span>					
			</p><!-- .form-input'.$id.' -->';
		$filterArray['datepickerCustomField'] = apply_filters('wppb_'.$page.'_datepicker_custom_field_'.$id, $filterArray['datepickerCustomField']);
...
function wppb_textarea_handler($page, $item_title, $id, $item_desc, $item_required, $item_id, $item_type, $currentUser, $filterArray, $extraFieldsErrorHolder, $postData, $item_options, $error, $fieldName){
	...
	if ($page == 'back_end'){
		$filterArray['textareaCustomField'] = '
			<table class="form-table">
				<tr>
					<th><label for="'.$item_title.'">'.$item_title.'</label></th>
					<td>
						<textarea rows="'.$item_options.'" name="'.$item_id.$id.'" id="'.$item_title.'" wrap="virtual">'.$inputValue.'</textarea><br/>
						<span class="description">'.$item_desc.'</span>
					</td>
				</tr>
			</table>';
		$filterArray['textareaCustomField'] = apply_filters('wppb_'.$page.'_textarea_custom_field_'.$id, $filterArray['textareaCustomField']);		
	}else{
		$errorVar = '';
		$errorMark = '';
		if (isset($item_required))
		...
		if ($page == 'register'){
			$inputValue = '';
			if (isset($_POST[$item_id.$id]))
				$inputValue = $_POST[$item_id.$id];
		}
		$filterArray['textareaCustomField'] = '
			<p class="form-textarea'.$id.$errorVar.'">
				<label for="'.$item_title.'">'.$item_title.$errorMark.'</label>
				<textarea rows="'.$item_options.'" name="'.$item_id.$id.'" id="'.$item_title.'" wrap="virtual">'.$inputValue.'</textarea>
...
function wppb_upload_handler($page, $item_title, $id, $item_desc, $item_required, $item_id, $item_type, $currentUser, $filterArray, $extraFieldsErrorHolder, $postData, $item_options, $error, $fieldName){
	
	//add the nonce field
	$wppb_nonce = wp_create_nonce( $page.'_nonce' );
	
	if (trim($item_desc) != '')
		$item_desc .= '<br/>';
	$imgSource = WPPB_PLUGIN_URL . '/assets/images/';
	$userData = get_user_meta($currentUser, $fieldName, true);
	$postion = strpos ( $userData , '_attachment_' );
	...
	if ($page == 'back_end'){
		$filterArray['uploadCustomField'] = '
			<table class="form-table">
				<tr>
					<th><label for="'.$item_title.'">'.$item_title.'</label></th>
					<td>
					<input name="'.$item_type.$id.'" size="40" type="file" /><font color="grey" size="1">('. __('max upload size', 'profilebuilder') .' '.WPPB_SERVER_MAX_UPLOAD_SIZE_MEGA.'b)</font>
		...
		$filterArray['uploadCustomField'] = '
			<p class="form-upload'.$id.$errorVar.'">
				<label for="'.$item_title.'">'.$item_title.$errorMark.'</label>
				<input name="'.$item_type.$id.'" size="30" type="file" /><span class="wppb-max-upload">('. __('max upload size', 'profilebuilder') .' '.WPPB_SERVER_MAX_UPLOAD_SIZE_MEGA.'b)</span>
				...
				if ($page != 'register'){
					if (($userData == '') || ($userData == get_bloginfo('url').'/wp-content/uploads/profile_builder/attachments/')){
						...
						if (($item_required !=null) && ($item_required == 'yes')){
							$filterArray['uploadCustomField'] .= '</span><br/><span class="wppb-description-delimiter"><u>'. __('Current file', 'profilebuilder') .'</u>: '.$fileName.'<a href="'.$userData.'" target="_blank" class="wppb-cattachment"><img src="'.$imgSource.'attachment.png" title="'. __('Click to see the current attachment', 'profilebuilder') .'"></a><img src="'.$imgSource.'icon_delete_disabled.png" title="The attachment can\'t be deleted (It was marked as required by the administrator)."></span>';
						}else{
							$filterArray['uploadCustomField'] .= '</span><br/><span class="wppb-description-delimiter"><u>'. __('Current file', 'profilebuilder') .'</u>: '.$fileName.'<a href="'.$userData.'" target="_blank" class="wppb-cattachment"><img src="'.$imgSource.'attachment.png" title="'. __('Click to see the current attachment', 'profilebuilder') .'"></a><a href="javascript:confirmDelete(\''.$wppb_nonce.'\',\''.$currentUser.'\',\''.$id.'\',\''.$fieldName.'\',\''.get_permalink().$conjure.'fileType=attachment&fileName='.$fileName.'\',\''.get_bloginfo('url').'/wp-admin/admin-ajax.php\',\'attachment\')" class="wppb-dattachment"><img src="'.$imgSource.'icon_delete.png" title="'. __('Click to delete the current attachment', 'profilebuilder') .'"></a></span>';
						}
					}
...
function wppb_avatar_handler($page, $item_title, $id, $item_desc, $item_required, $item_id, $item_type, $currentUser, $filterArray, $extraFieldsErrorHolder, $postData, $item_options, $error, $fieldName){
	...
	if ($page == 'back_end'){
		$filterArray['avatarCustomField'] = '
			<table class="form-table">
				<tr>
					<th><label for="'.$item_type.'">'.$item_title.'</label></th>
					<td>
						<input name="'.$item_type.$id.'" size="40" type="file" /><font color="grey" size="1">('. __('max upload size', 'profilebuilder') .' '.WPPB_SERVER_MAX_UPLOAD_SIZE_MEGA.'b)</font>
						<br/><span class="wppb-description-delimiter">'.$item_desc;
...
function wppb_agree_to_terms_handler($page, $item_title, $id, $item_desc, $item_required, $item_id, $item_type, $currentUser, $filterArray, $extraFieldsErrorHolder, $postData, $item_options, $error, $fieldName){
	...
	if ($page == 'register'){
		...
		if (isset($value['item_required']))
			if ($value['item_required'] == 'yes')
				$errorMark = '<font color="red" title="'. __('This field is marked as required by the administrator.', 'profilebuilder') .'">*</font>';
		if (in_array($value['id'], $extraFieldsErrorHolder))
			$errorVar = ' errorHolder';
		$filterArray['agreeToTermsCustomField'] = '
			<p class="form-checkbox'.$id.$errorVar.'">
					<label for="'.$item_title.'">'.$item_title.$errorMark.'</label>';						
					$filterArray['agreeToTermsCustomField'] .= '<input value="agree" name="'.$item_id.$id.'" type="checkbox"'; 
					if ( $error && ( $_POST[$item_id.$id] == 'agree' ) )
		...
		$filterArray['agreeToTermsCustomField'] = apply_filters('wppb_'.$page.'_upload_custom_field_'.$id, $filterArray['agreeToTermsCustomField']);		
	}
	...
	if (count($wppbFetchArray) >= 1){
		foreach($wppbFetchArray as $key => $value){
			switch ($value['item_type']) {
				...
				case "input":{
					$customFieldsArray[$value['item_metaName']] = wppb_input_handler($page, $value['item_title'], $value['id'], $value['item_desc'], $value['item_required'], $value['item_id'], $value['item_type'], $current_user, $extraFieldsFilterArray, $extraFieldsErrorHolder, $postData, $value['item_options'], $error, $value['item_metaName']);
					break;
				}
				case "hiddenInput":{
					$customFieldsArray[$value['item_metaName']] = wppb_hidden_input_handler($page, $value['item_title'], $value['id'], $value['item_desc'], $value['item_required'], $value['item_id'], $value['item_type'], $current_user, $extraFieldsFilterArray, $extraFieldsErrorHolder, $postData, $value['item_options'], $error, $value['item_metaName']);
					break;
				}
				case "checkbox":{
					$customFieldsArray[$value['item_metaName']] = wppb_checkbox_handler($page, $value['item_title'], $value['id'], $value['item_desc'], $value['item_required'], $value['item_id'], $value['item_type'], $current_user, $extraFieldsFilterArray, $extraFieldsErrorHolder, $postData, $value['item_options'], $error, $value['item_metaName']);
					break;
				}
				case "radio":{
					$customFieldsArray[$value['item_metaName']] = wppb_radio_handler($page, $value['item_title'], $value['id'], $value['item_desc'], $value['item_required'], $value['item_id'], $value['item_type'], $current_user, $extraFieldsFilterArray, $extraFieldsErrorHolder, $postData, $value['item_options'], $error, $value['item_metaName']);
					break;
				}
				case "select":{
					$customFieldsArray[$value['item_metaName']] = wppb_select_handler($page, $value['item_title'], $value['id'], $value['item_desc'], $value['item_required'], $value['item_id'], $value['item_type'], $current_user, $extraFieldsFilterArray, $extraFieldsErrorHolder, $postData, $value['item_options'], $error, $value['item_metaName']);
					break;
				}
				case "countrySelect":{
					$customFieldsArray[$value['item_metaName']] = wppb_country_select_handler($page, $value['item_title'], $value['id'], $value['item_desc'], $value['item_required'], $value['item_id'], $value['item_type'], $current_user, $extraFieldsFilterArray, $extraFieldsErrorHolder, $postData, $value['item_options'], $error, $value['item_metaName']);
					break;
				}
				case "timeZone":{
					$customFieldsArray[$value['item_metaName']] = wppb_timezone_select_handler($page, $value['item_title'], $value['id'], $value['item_desc'], $value['item_required'], $value['item_id'], $value['item_type'], $current_user, $extraFieldsFilterArray, $extraFieldsErrorHolder, $postData, $value['item_options'], $error, $value['item_metaName']);
					break;
				}
				case "datepicker":{
					$customFieldsArray[$value['item_metaName']] = wppb_datepicker_handler($page, $value['item_title'], $value['id'], $value['item_desc'], $value['item_required'], $value['item_id'], $value['item_type'], $current_user, $extraFieldsFilterArray, $extraFieldsErrorHolder, $postData, $value['item_options'], $error, $value['item_metaName']);
					break;
				}
				case "textarea":{
					$customFieldsArray[$value['item_metaName']] = wppb_textarea_handler($page, $value['item_title'], $value['id'], $value['item_desc'], $value['item_required'], $value['item_id'], $value['item_type'], $current_user, $extraFieldsFilterArray, $extraFieldsErrorHolder, $postData, $value['item_options'], $error, $value['item_metaName']);
					break;
				}
				case "upload":{
					$customFieldsArray[$value['item_metaName']] .= wppb_upload_handler($page, $value['item_title'], $value['id'], $value['item_desc'], $value['item_required'], $value['item_id'], $value['item_type'], $current_user, $extraFieldsFilterArray, $extraFieldsErrorHolder, $postData, $value['item_options'], $error, $value['item_metaName']);
					break;
				}
				case "avatar":{
					$customFieldsArray[$value['item_metaName']] .= wppb_avatar_handler($page, $value['item_title'], $value['id'], $value['item_desc'], $value['item_required'], $value['item_id'], $value['item_type'], $current_user, $extraFieldsFilterArray, $extraFieldsErrorHolder, $postData, $value['item_options'], $error, $value['item_metaName']);
					break;
				}
				case "agreeToTerms":{
					$customFieldsArray[$value['item_metaName']] .= wppb_agree_to_terms_handler($page, $value['item_title'], $value['id'], $value['item_desc'], $value['item_required'], $value['item_id'], $value['item_type'], $current_user, $extraFieldsFilterArray, $extraFieldsErrorHolder, $postData, $value['item_options'], $error, $value['item_metaName']);
					break;
				}
			}
		}
	...
	if (count($wppbFetchArray) >= 1){
		foreach($wppbFetchArray as $key => $value){
			switch ($value['item_type']) {
				...
				case "input":{
					$customFieldsArray[$value['item_metaName']] = wppb_input_handler($page, $value['item_title'], $value['id'], $value['item_desc'], $value['item_required'], $value['item_id'], $value['item_type'], $user->ID, $extraFieldsFilterArray, $extraFieldsErrorHolder, $postData, $value['item_options'], '',$value['item_metaName']);
					break;
				}
				case "hiddenInput":{
					$customFieldsArray[$value['item_metaName']] = wppb_hidden_input_handler($page, $value['item_title'], $value['id'], $value['item_desc'], $value['item_required'], $value['item_id'], $value['item_type'], $user->ID, $extraFieldsFilterArray, $extraFieldsErrorHolder, $postData, $value['item_options'], '', $value['item_metaName']);
					break;
				}
				case "checkbox":{
					$customFieldsArray[$value['item_metaName']] = wppb_checkbox_handler($page, $value['item_title'], $value['id'], $value['item_desc'], $value['item_required'], $value['item_id'], $value['item_type'], $user->ID, $extraFieldsFilterArray, $extraFieldsErrorHolder, $postData, $value['item_options'], '', $value['item_metaName']);
					break;
				}
				case "radio":{
					$customFieldsArray[$value['item_metaName']] = wppb_radio_handler($page, $value['item_title'], $value['id'], $value['item_desc'], $value['item_required'], $value['item_id'], $value['item_type'], $user->ID, $extraFieldsFilterArray, $extraFieldsErrorHolder, $postData, $value['item_options'], '', $value['item_metaName']);
					break;
				}
				case "select":{
					$customFieldsArray[$value['item_metaName']] = wppb_select_handler($page, $value['item_title'], $value['id'], $value['item_desc'], $value['item_required'], $value['item_id'], $value['item_type'], $user->ID, $extraFieldsFilterArray, $extraFieldsErrorHolder, $postData, $value['item_options'], '', $value['item_metaName']);
					break;
				}
				case "countrySelect":{
					$customFieldsArray[$value['item_metaName']] = wppb_country_select_handler($page, $value['item_title'], $value['id'], $value['item_desc'], $value['item_required'], $value['item_id'], $value['item_type'], $user->ID, $extraFieldsFilterArray, $extraFieldsErrorHolder, $postData, $value['item_options'], '', $value['item_metaName']);
					break;
				}
				case "timeZone":{
					$customFieldsArray[$value['item_metaName']] = wppb_timezone_select_handler($page, $value['item_title'], $value['id'], $value['item_desc'], $value['item_required'], $value['item_id'], $value['item_type'], $user->ID, $extraFieldsFilterArray, $extraFieldsErrorHolder, $postData, $value['item_options'], '', $value['item_metaName']);
					break;
				}
				case "datepicker":{
					$customFieldsArray[$value['item_metaName']] = wppb_datepicker_handler($page, $value['item_title'], $value['id'], $value['item_desc'], $value['item_required'], $value['item_id'], $value['item_type'], $user->ID, $extraFieldsFilterArray, $extraFieldsErrorHolder, $postData, $value['item_options'], '', $value['item_metaName']);
					break;
				}
				case "textarea":{
					$customFieldsArray[$value['item_metaName']] = wppb_textarea_handler($page, $value['item_title'], $value['id'], $value['item_desc'], $value['item_required'], $value['item_id'], $value['item_type'], $user->ID, $extraFieldsFilterArray, $extraFieldsErrorHolder, $postData, $value['item_options'], '', $value['item_metaName']);
					break;
				}
				case "upload":{
					$customFieldsArray[$value['item_metaName']] = wppb_upload_handler($page, $value['item_title'], $value['id'], $value['item_desc'], $value['item_required'], $value['item_id'], $value['item_type'], $user->ID, $extraFieldsFilterArray, $extraFieldsErrorHolder, $postData, $value['item_options'], '', $value['item_metaName']);
					break;
				}
				case "avatar":{
					$customFieldsArray[$value['item_metaName']] = wppb_avatar_handler($page, $value['item_title'], $value['id'], $value['item_desc'], $value['item_required'], $value['item_id'], $value['item_type'], $user->ID, $extraFieldsFilterArray, $extraFieldsErrorHolder, $postData, $value['item_options'], '', $value['item_metaName']);
					break;
				}
			}
		}
...
function save_profile_extra_fields_in_admin($user_id){
	...
	foreach ( $wppbFetchArray as $key => $value){
		switch ($value['item_type']) {
			case "input":{
				update_user_meta( $user_id, $value['item_metaName'], esc_attr( $_POST[$value['item_id'].$value['id']] ) );
				break;
			}			
			case "hiddenInput":{
				update_user_meta( $user_id, $value['item_metaName'], esc_attr( $_POST[$value['item_id'].$value['id']] ) );
			...
			case "radio":{
				update_user_meta( $user_id, $value['item_metaName'], $_POST[$value['item_id'].$value['id']]  );
				break;
			}
			case "select":{
				update_user_meta( $user_id, $value['item_metaName'], esc_attr( $_POST[$value['item_id'].$value['id']] ) );
