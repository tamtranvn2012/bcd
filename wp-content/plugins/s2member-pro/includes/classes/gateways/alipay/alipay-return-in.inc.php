										...
										if (preg_match ("/^(TRADE_FINISHED|TRADE_SUCCESS)$/i", $alipay["trade_status"]) && !$alipay["refund_status"])
											{
												...
												/**/
												list ($alipay["time"], $alipay["item_number"], $alipay["referencing"], $alipay["customer_ip"]) = preg_split ("/~/", $alipay["out_trade_no"]);
												...
												/**/
												$alipay["s2member_log"][] = add_query_arg (urlencode_deep ($rtn), site_url ("/?s2member_pro_alipay_return&s2member_paypal_return=1" . $rtn_q));
												/**/
												wp_redirect(add_query_arg (urlencode_deep ($rtn), site_url ("/?s2member_pro_alipay_return&s2member_paypal_return=1" . $rtn_q)));
											}
										else
											{
												...
												echo "alert('ERROR: Unexpected status. Please contact Support for assistance.\\n\\nThe AliPay® status did NOT match a required action.');" . "\n";
