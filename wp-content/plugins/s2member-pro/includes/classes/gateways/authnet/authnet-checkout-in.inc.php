								...
								if (!c_ws_plugin__s2member_pro_authnet_responses::authnet_form_attr_validation_errors ($post_vars["attr"])) /* Attr errors? */
									{
										if (!($error = c_ws_plugin__s2member_pro_authnet_responses::authnet_form_submission_validation_errors ("checkout", $post_vars)))
											{
												$cp_attr = c_ws_plugin__s2member_pro_authnet_utilities::authnet_apply_coupon ($post_vars["attr"], $post_vars["coupon"]);
												...
												if ($use_recurring_profile && is_user_logged_in () && is_object ($user = wp_get_current_user ()) && ($user_id = $user->ID))
													{
														...
														/**/
														$full_name = trim ($post_vars["first_name"] . " " . $post_vars["last_name"]);
														/**/
																...
																if (($authnet = c_ws_plugin__s2member_pro_authnet_utilities::authnet_arb_response ($authnet)) && (empty ($authnet["__error"]) || ($_authnet && !empty ($_authnet["transaction_id"]) && $authnet["response_reason_code"] === "E00018")))
																	{
																		...
																		if (!($ipn = array ())) /* Simulated PayPal® IPN. */
																			{
																				...
																				/**/
																				$ipn["s2member_authnet_proxy_return_url"] = c_ws_plugin__s2member_utils_urls::remote (site_url ("/?s2member_paypal_notify=1" . $ipn_q), $ipn, array ("timeout" => 20));
																			}
																		/**/
																		...
																		/**/
																		$global_response = array ("response" => '<strong>Thank you.</strong> Your account has been updated.<br />&mdash; Please <a href="' . esc_attr (wp_login_url ()) . '" rel="nofollow">log back in</a> now.');
																		/**/
																		if ($post_vars["attr"]["success"] && substr ($ipn["s2member_authnet_proxy_return_url"], 0, 2) === substr ($post_vars["attr"]["success"], 0, 2) /* In case the IPN routine returns something weird. */
																		&& ($custom_success_url = preg_replace ("/%%response%%/i", urlencode ($global_response["response"]), $ipn["s2member_authnet_proxy_return_url"])) /* Response message that would have been displayed. */
																		&& ($custom_success_url = trim (preg_replace ("/%%(.+?)%%/i", "", $custom_success_url))) /* Now we clean up any remaining Replacement Codes. We need to do this here; the IPN handler expects this. */
																		&& wp_redirect (c_ws_plugin__s2member_pro_authnet_utilities::authnet_s2p_v_generate ($custom_success_url)) !== "nill") /* Also generate an s2p-v variable; used for verification of query string vars. */
																			exit ();
																	}
																else /* Else, an error. */
																	{
																				...
																				if (((is_multisite () && ($new__user_id = c_ws_plugin__s2member_registrations::ms_create_existing_user ($create_user["user_login"], $create_user["user_email"], $create_user["user_pass"]))) || ($new__user_id = wp_create_user ($create_user["user_login"], $create_user["user_pass"], $create_user["user_email"]))) && !is_wp_error ($new__user_id))
																					{
																						...
																						/**/
																						$global_response = array ("response" => '<strong>Thank you.</strong> Your account has been approved.<br />&mdash; Please <a href="' . esc_attr (wp_login_url ()) . '" rel="nofollow">login</a>.');
																						/**/
																						if ($post_vars["attr"]["success"] && substr ($ipn["s2member_authnet_proxy_return_url"], 0, 2) === substr ($post_vars["attr"]["success"], 0, 2) /* In case the IPN routine returns something weird. */
																						&& ($custom_success_url = preg_replace ("/%%response%%/i", urlencode ($global_response["response"]), $ipn["s2member_authnet_proxy_return_url"])) /* Response message that would have been displayed. */
																						&& ($custom_success_url = trim (preg_replace ("/%%(.+?)%%/i", "", $custom_success_url))) /* Now we clean up any remaining Replacement Codes. We need to do this here; the IPN handler expects this. */
																				...
																				else /* Else, an error reponse should be given. */
																					{
																						...
																						/**/
																						$global_response = array ("response" => 'Oops. A slight problem. Please contact Support for assistance.', "error" => true);
																					}
																			}
																				...
																				if (((is_multisite () && ($new__user_id = c_ws_plugin__s2member_registrations::ms_create_existing_user ($create_user["user_login"], $create_user["user_email"], $create_user["user_pass"]))) || ($new__user_id = wp_create_user ($create_user["user_login"], $create_user["user_pass"], $create_user["user_email"]))) && !is_wp_error ($new__user_id))
																					{
																						...
																						if ($_authnet && !empty ($_authnet["transaction_id"]) && $authnet["response_reason_code"] === "E00018")
																							update_user_option ($new__user_id, "s2member_auto_eot_time", $start_time);
																						/**/
																						$global_response = array ("response" => '<strong>Thank you.</strong> Your account has been approved.<br />&mdash; You\'ll receive an email momentarily.');
																						/**/
																						if ($post_vars["attr"]["success"] && substr ($ipn["s2member_authnet_proxy_return_url"], 0, 2) === substr ($post_vars["attr"]["success"], 0, 2) /* In case the IPN routine returns something weird. */
																						&& ($custom_success_url = preg_replace ("/%%response%%/i", urlencode ($global_response["response"]), $ipn["s2member_authnet_proxy_return_url"])) /* Response message that would have been displayed. */
																						&& ($custom_success_url = trim (preg_replace ("/%%(.+?)%%/i", "", $custom_success_url))) /* Now we clean up any remaining Replacement Codes. We need to do this here; the IPN handler expects this. */
												...
												/**/
													...
													{
														$global_response = array ("response" => 'Unknown error. Please contact Support for assistance.', "error" => true);
													}
											}
										else /* Else, an error. */
											{
