				...
				public static function sc_authnet_form ($attr = FALSE, $content = FALSE, $shortcode = FALSE)
					{
						...
						$attr["cc"] = "USD"; /* Authorize.Net® always processes in USD. International transactions are converted automatically to USD. */
						...
						/**/
						$attr["accept"] = (trim ($attr["accept"])) ? preg_split ("/[;,]+/", preg_replace ("/[\r\n\t\s]+/", "", strtolower ($attr["accept"]))) : array (); /* Convert to array. */
						$attr["accept"] = (empty ($attr["accept"])) ? array_merge ($attr["accept"], array ("visa")) : $attr["accept"]; /* Must always accept at least one. Default to visa. */
						/**/
						...
						if ($attr["cancel"]) /* This handles cancellation form construction. */
							{
								...
								*/
								$response = c_ws_plugin__s2member_pro_authnet_responses::authnet_cancellation_response ($attr); /* Pre-formatted response. */
								/*
								Empty post vars on successful response.
								*/
								$_p = ($response["response"] && !$response["error"]) ? array (): $_p; /* Avoid duplicate submissions. */
								/*
								...
								*/
								$custom_template = (file_exists (TEMPLATEPATH . "/authnet-cancellation-form.html")) ? TEMPLATEPATH . "/authnet-cancellation-form.html" : false;
								...
								/**/
								$code = trim (file_get_contents ((($custom_template) ? $custom_template : dirname (dirname (dirname (dirname (__FILE__)))) . "/templates/forms/authnet-cancellation-form.html")));
						...
						*/
						else if ($attr["register"]) /* This handles free registration forms. */
							{
								...
								Obtain a possible response and/or validation error.
								*/
								$response = c_ws_plugin__s2member_pro_authnet_responses::authnet_registration_response ($attr); /* Pre-formatted response. */
								/*
								Empty post vars on successful response.
								*/
								$_p = ($response["response"] && !$response["error"]) ? array (): $_p; /* Avoid duplicate submissions. */
													...
													if (in_array ($field["id"], $fields_applicable)) /* Field is applicable to Level 0? */
														{
															...
															$custom_fields .= c_ws_plugin__s2member_custom_reg_fields::custom_field_gen (__FUNCTION__, $field, "s2member_pro_authnet_registration[custom_fields][", "s2member-pro-authnet-registration-custom-reg-field-", "s2member-pro-authnet-custom-reg-field-" . $field_id_class . " s2member-pro-authnet-registration-custom-reg-field-" . $field_id_class, "", ($tabindex = $tabindex + 10), "", $_p, $_p["s2member_pro_authnet_registration"]["custom_fields"][$field_var], "registration");
															/**/
															$custom_fields .= '</div>' . "\n";
														}
												}
								...
								*/
								...
								Get the form template.
								*/
								$custom_template = (file_exists (TEMPLATEPATH . "/authnet-registration-form.html")) ? TEMPLATEPATH . "/authnet-registration-form.html" : false;
								$custom_template = ($attr["template"] && file_exists (TEMPLATEPATH . "/" . $attr["template"])) ? TEMPLATEPATH . "/" . $attr["template"] : $custom_template;
								$custom_template = ($attr["template"] && file_exists (WP_CONTENT_DIR . "/" . $attr["template"])) ? WP_CONTENT_DIR . "/" . $attr["template"] : $custom_template;
								/**/
								$code = trim (file_get_contents ((($custom_template) ? $custom_template : dirname (dirname (dirname (dirname (__FILE__)))) . "/templates/forms/authnet-registration-form.html")));
								$code = (!is_multisite () || !c_ws_plugin__s2member_utils_conds::is_multisite_farm () || is_main_site ()) ? c_ws_plugin__s2member_utilities::evl ($code) : $code;
								/*
								Fill in the action.
								*/
						...
						*/
						else if ($attr["update"]) /* This handles billing information updates. */
							{
								...
								Obtain a possible response and/or validation error.
								*/
								$response = c_ws_plugin__s2member_pro_authnet_responses::authnet_update_response ($attr); /* Pre-formatted response. */
								/*
								Empty post vars on successful response.
								*/
								$_p = ($response["response"] && !$response["error"]) ? array (): $_p; /* Avoid duplicate submissions. */
								...
								*/
								...
								Get the form template.
								*/
								$custom_template = (file_exists (TEMPLATEPATH . "/authnet-update-form.html")) ? TEMPLATEPATH . "/authnet-update-form.html" : false;
								$custom_template = ($attr["template"] && file_exists (TEMPLATEPATH . "/" . $attr["template"])) ? TEMPLATEPATH . "/" . $attr["template"] : $custom_template;
								$custom_template = ($attr["template"] && file_exists (WP_CONTENT_DIR . "/" . $attr["template"])) ? WP_CONTENT_DIR . "/" . $attr["template"] : $custom_template;
								/**/
								$code = trim (file_get_contents ((($custom_template) ? $custom_template : dirname (dirname (dirname (dirname (__FILE__)))) . "/templates/forms/authnet-update-form.html")));
								$code = (!is_multisite () || !c_ws_plugin__s2member_utils_conds::is_multisite_farm () || is_main_site ()) ? c_ws_plugin__s2member_utilities::evl ($code) : $code;
								/*
								Fill in the action.
								*/
						...
						*/
						else if ($attr["sp"]) /* This handles Specific Post/Page Access. No registration required. */
							{
								...
								*/
								...
								Obtain a possible response and/or validation error.
								*/
								$response = c_ws_plugin__s2member_pro_authnet_responses::authnet_sp_checkout_response ($attr); /* Pre-formatted response. */
								/*
								Empty post vars on successful response.
								*/
								$_p = ($response["response"] && !$response["error"]) ? array (): $_p; /* Avoid duplicate submissions. */
								...
								*/
								...
								Get the form template.
								*/
								$custom_template = (file_exists (TEMPLATEPATH . "/authnet-sp-checkout-form.html")) ? TEMPLATEPATH . "/authnet-sp-checkout-form.html" : false;
								$custom_template = ($attr["template"] && file_exists (TEMPLATEPATH . "/" . $attr["template"])) ? TEMPLATEPATH . "/" . $attr["template"] : $custom_template;
								$custom_template = ($attr["template"] && file_exists (WP_CONTENT_DIR . "/" . $attr["template"])) ? WP_CONTENT_DIR . "/" . $attr["template"] : $custom_template;
								/**/
								$code = trim (file_get_contents ((($custom_template) ? $custom_template : dirname (dirname (dirname (dirname (__FILE__)))) . "/templates/forms/authnet-sp-checkout-form.html")));
								$code = (!is_multisite () || !c_ws_plugin__s2member_utils_conds::is_multisite_farm () || is_main_site ()) ? c_ws_plugin__s2member_utilities::evl ($code) : $code;
								/*
								Fill in the action.
								*/
								...
								*/
								$code = preg_replace ("/%%coupon_response%%/", c_ws_plugin__s2member_utils_strings::esc_ds (c_ws_plugin__s2member_pro_authnet_utilities::authnet_apply_coupon ($attr, $attr["coupon"], "response")), $code);
						...
						*/
						else /* This handles initial purchase, and also modification checkout forms. */
							{
								...
								*/
								...
								Obtain a possible response and/or validation error.
								*/
								$response = c_ws_plugin__s2member_pro_authnet_responses::authnet_checkout_response ($attr); /* Pre-formatted response. */
								/*
								Empty post vars on successful response.
								*/
													...
													if (in_array ($field["id"], $fields_applicable)) /* Field is applicable to this Level? */
														{
															...
															$custom_fields .= '<span' . ((preg_match ("/^(checkbox|pre_checkbox)$/", $field["type"])) ? ' style="display:none;"' : '') . '>' . $field["label"] . (($field["required"] === "yes") ? ' *' : '') . '</span></label>' . ((preg_match ("/^(checkbox|pre_checkbox)$/", $field["type"])) ? '' : '<br />') . "\n";
															$custom_fields .= c_ws_plugin__s2member_custom_reg_fields::custom_field_gen (__FUNCTION__, $field, "s2member_pro_authnet_checkout[custom_fields][", "s2member-pro-authnet-checkout-custom-reg-field-", "s2member-pro-authnet-custom-reg-field-" . $field_id_class . " s2member-pro-authnet-checkout-custom-reg-field-" . $field_id_class, "", ($tabindex = $tabindex + 10), "", $_p, $_p["s2member_pro_authnet_checkout"]["custom_fields"][$field_var], "registration");
															/**/
															$custom_fields .= '</div>' . "\n";
														}
												}
								...
								*/
								...
								Get the form template.
								*/
								$custom_template = (file_exists (TEMPLATEPATH . "/authnet-checkout-form.html")) ? TEMPLATEPATH . "/authnet-checkout-form.html" : false;
								$custom_template = ($attr["template"] && file_exists (TEMPLATEPATH . "/" . $attr["template"])) ? TEMPLATEPATH . "/" . $attr["template"] : $custom_template;
								$custom_template = ($attr["template"] && file_exists (WP_CONTENT_DIR . "/" . $attr["template"])) ? WP_CONTENT_DIR . "/" . $attr["template"] : $custom_template;
								/**/
								$code = trim (file_get_contents ((($custom_template) ? $custom_template : dirname (dirname (dirname (dirname (__FILE__)))) . "/templates/forms/authnet-checkout-form.html")));
								$code = (!is_multisite () || !c_ws_plugin__s2member_utils_conds::is_multisite_farm () || is_main_site ()) ? c_ws_plugin__s2member_utilities::evl ($code) : $code;
								/*
								Fill in the action.
								*/
