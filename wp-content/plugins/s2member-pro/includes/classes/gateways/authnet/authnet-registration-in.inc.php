...
/**/
if (!class_exists ("c_ws_plugin__s2member_pro_authnet_registration_in"))
	{
		/**
		* Authorize.Net® Registration Form ( inner processing routines ).
		*
		...
		* @since 1.5
		*/
		class c_ws_plugin__s2member_pro_authnet_registration_in
			{
				/**
														...
														if (((is_multisite () && ($new__user_id = c_ws_plugin__s2member_registrations::ms_create_existing_user ($create_user["user_login"], $create_user["user_email"], $create_user["user_pass"]))) || ($new__user_id = wp_create_user ($create_user["user_login"], $create_user["user_pass"], $create_user["user_email"]))) && !is_wp_error ($new__user_id))
															{
																...
																/**/
																$global_response = array ("response" => '<strong>Thank you.</strong> Please <a href="' . esc_attr (wp_login_url ()) . '" rel="nofollow">login</a>.');
																/**/
																if ($post_vars["attr"]["success"] && substr ($GLOBALS["ws_plugin__s2member_registration_return_url"], 0, 2) === substr ($post_vars["attr"]["success"], 0, 2) /* In case the registration routine returns something weird. */
																&& ($custom_success_url = preg_replace ("/%%response%%/i", urlencode ($global_response["response"]), $GLOBALS["ws_plugin__s2member_registration_return_url"])) /* Response message that would have been displayed. */
																&& ($custom_success_url = trim (preg_replace ("/%%(.+?)%%/i", "", $custom_success_url))) /* Now we clean up any remaining Replacement Codes. We need to do this here; the registration handler expects this. */
														...
														if (((is_multisite () && ($new__user_id = c_ws_plugin__s2member_registrations::ms_create_existing_user ($create_user["user_login"], $create_user["user_email"], $create_user["user_pass"]))) || ($new__user_id = wp_create_user ($create_user["user_login"], $create_user["user_pass"], $create_user["user_email"]))) && !is_wp_error ($new__user_id))
															{
																...
																wp_new_user_notification ($new__user_id, $create_user["user_pass"]);
																/**/
																$global_response = array ("response" => '<strong>Thank you.</strong> You\'ll receive an email momentarily.');
																/**/
																if ($post_vars["attr"]["success"] && substr ($GLOBALS["ws_plugin__s2member_registration_return_url"], 0, 2) === substr ($post_vars["attr"]["success"], 0, 2) /* In case the registration routine returns something weird. */
																&& ($custom_success_url = preg_replace ("/%%response%%/i", urlencode ($global_response["response"]), $GLOBALS["ws_plugin__s2member_registration_return_url"])) /* Response message that would have been displayed. */
																&& ($custom_success_url = trim (preg_replace ("/%%(.+?)%%/i", "", $custom_success_url))) /* Now we clean up any remaining Replacement Codes. We need to do this here; the registration handler expects this. */
