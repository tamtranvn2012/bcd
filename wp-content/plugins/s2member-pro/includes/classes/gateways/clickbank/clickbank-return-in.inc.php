										...
										if (is_array ($order = json_decode (c_ws_plugin__s2member_utils_urls::remote ("https://api.clickbank.com/rest/1.2/orders/" . $clickbank["cbreceipt"], false, array_merge (c_ws_plugin__s2member_pro_clickbank_utilities::clickbank_api_headers (), array ("timeout" => 20))), true)) && ($order = $order["orderData"]))
											{
												...
												/**/
												foreach ($clickbank as $var => $value) /* Pulls out the s2_ variables from Return Data. */
													if (preg_match ("/^s2_/", $var))
														$s2vars[$var] = $value;
												/**/
												if (isset ($s2vars["s2_p1"]) && $s2vars["s2_p1"] === "0 D") /* Initial Period. */
													$s2vars["s2_p1"] = $s2vars["s2_p3"]; /* No Trial defaults to Regular Period. */
												/**/
												$clickbank["s2vars"] = $s2vars; /* So they appear in the log entry for this Notification. */
												/**/
												if (preg_match ("/^(TEST_)?SALE$/i", $order["txnType"]) && !(isset ($s2vars["s2_p1"]) && isset ($s2vars["s2_p3"])))
													{
														...
														/**/
														$clickbank["s2member_log"][] = add_query_arg (urlencode_deep ($rtn), site_url ("/?s2member_pro_clickbank_return&s2member_paypal_return=1" . $rtn_q));
														/**/
														wp_redirect(add_query_arg (urlencode_deep ($rtn), site_url ("/?s2member_pro_clickbank_return&s2member_paypal_return=1" . $rtn_q)));
													}
												else if (preg_match ("/^(TEST_)?SALE$/i", $order["txnType"]) && (isset ($s2vars["s2_p1"]) && isset ($s2vars["s2_p3"])))
													{
												...
												else
													{
														...
														echo "alert('ERROR: Unexpected txnType. Please contact Support for assistance.\\n\\nThe ClickBank® txnType did not match a required action.');" . "\n";
										...
										else /* Extensive log reporting here. Sometimes it takes a few seconds for the ClickBank® API to receive data for new orders. This is here in case that happens. */
											{
												...
												if (!empty ($_GET["s2member_pro_clickbank_return_success"]))
													$rtn_q .= "&s2member_paypal_return_success=" . rawurlencode (trim (stripslashes ($_GET["s2member_pro_clickbank_return_success"])));
												/**/
												$clickbank["s2member_log"][] = site_url ("/?s2member_pro_clickbank_return&s2member_paypal_return=1" . $rtn_q);
												/**/
												wp_redirect(site_url ("/?s2member_pro_clickbank_return&s2member_paypal_return=1" . $rtn_q));
											}
									}
