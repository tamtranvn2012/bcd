						...
						if (!empty ($_REQUEST["s2member_pro_clickbank_return"]) && !empty ($_REQUEST["cbpop"]))
							{
								...
								/**/
								$mb = function_exists ("mb_convert_encoding") ? @mb_convert_encoding ($cbpop, "UTF-8") : $cbpop;
						...
						else if (!empty ($_REQUEST["s2member_pro_clickbank_notify"]) && !empty ($_REQUEST["cverify"]))
							{
								...
								/**/
								$mb = function_exists ("mb_convert_encoding") ? @mb_convert_encoding ($cverify, "UTF-8") : $cverify;
				...
				public static function clickbank_cc_reminder ($support = FALSE, $vars = FALSE)
					{
						if (!empty ($vars["template"]) && $vars["template"] === "clickbank")
							...
							'<div class="cc-reminder"><strong>Reminder:</strong> Purchases at this site will appear on your credit card or bank statement as: <code>ClickBank®</code> or <code>CLKBANK*COM</code>.</div>';
						/**/
						return $support;
					}
			}
