				...
				public static function google_co ()
					{
						...
						/**/
						if (!empty ($_GET["s2member_pro_google_co"]) && !empty ($_REQUEST["co"]) && is_array ($attr = c_ws_plugin__s2member_utils_strings::trim_deep (stripslashes_deep ($_REQUEST["co"]))) && $GLOBALS["WS_PLUGIN__"]["s2member"]["o"]["pro_google_merchant_id"])
							{
								...
								/**/
								else if ($attr["sp"]) /* This is a special routine for Specific Posts/Pages. These use a different XML layout/model. */
									{
										$attr["uniqid"] = uniqid (); /* Unique ID. Used in the Transaction ID for this Customer. */
										$attr["referencing"] = c_ws_plugin__s2member_utils_users::get_user_subscr_or_wp_id (); /* User/Member? */
										/**/
										...
										$xml .= '<item-description>(TID:s2-' . esc_html ($attr["uniqid"]) . ') Grants you immediate access.</item-description>';
										...
										/**/
										$xml .= '<digital-content>';
										$xml .= '<display-disposition>OPTIMISTIC</display-disposition>';
										...
										$endpoint = ($GLOBALS["WS_PLUGIN__"]["s2member"]["o"]["pro_google_sandbox"]) ? "sandbox.google.com/checkout" : "checkout.google.com";
										/**/
										if (($xml = c_ws_plugin__s2member_utils_urls::remote ("https://" . $endpoint . "/api/checkout/v2/merchantCheckout/Merchant/" . $GLOBALS["WS_PLUGIN__"]["s2member"]["o"]["pro_google_merchant_id"], $xml, array_merge (c_ws_plugin__s2member_pro_google_utilities::google_api_headers (), array ("timeout" => 20))))/**/
										/**/ && preg_match ("/\<redirect-url\>(.+?)\<\/redirect-url\>/i", preg_replace ("/[\r\n\t]+/", "", $xml), $m) && ($google = $m[1]))
											wp_redirect (wp_specialchars_decode ($google, ENT_QUOTES)); /* Redirect to Google® Checkout. */
										else /* Display error message. */
											echo strip_tags ($xml);
									}
								/**/
								else if ($attr["rr"] === "BN" || (!$attr["tp"] && !$attr["rr"])) /* This is a special routine for Buy Now Subscriptions. */
									{
										...
										if ($attr["referencing"]) /* If we're updating an existing account that is already in the system. */
											{
												$xml .= '<description>' . esc_html ('You\'ve been updated to:<br />') ./**/
												esc_html ($attr["desc"]) . /* Include desc here so the Customer can see that an update is taking place. */
												esc_html ('<br />( <a href="' . esc_attr (wp_login_url ()) . '">please log back in now</a> )') . '</description>';
												$xml .= '<url>' . esc_html (wp_login_url ()) . '</url>'; /* Also provide login URL in this link. */
											}
										else /* Otherwise, this checkout experience will establish a brand new Membership. */
											{
												$xml .= '<description>' . esc_html ($attr["desc"]) . /* Registration now. */
												esc_html ('<br />( the next step is to Register a Username )') . '</description>';
								...
								/**/
								else /* Else, it's an actual "Subscription"; use Membership routines. This may or may NOT be associated w/ recurring charges. */
									{
										$attr["desc"] = (!$attr["desc"]) ? $GLOBALS["WS_PLUGIN__"]["s2member"]["o"]["level" . $attr["level"] . "_label"] : $attr["desc"];
										/**/
										$attr["uniqid"] = uniqid (); /* Unique ID. Used as the Subscription ID for this Customer. */
										$attr["referencing"] = c_ws_plugin__s2member_utils_users::get_user_subscr_or_wp_id (); /* User/Member? */
										/**/
										...
										$attr["register_access_link"] = c_ws_plugin__s2member_register_access::register_link_gen ("google", "s2-" . $attr["uniqid"], $attr["custom"], $attr["level_ccaps_eotper"]);
										/**/
										if ($attr["tp"]) /* An actual Subscription that includes a Trial Period; which MIGHT also be associated with multiple recurring charges. */
											{
												...
												'<item-description>(SID:s2-' . esc_html ($attr["uniqid"]) . ') 100% free trial. NO charge today.</item-description>' :/**/
												'<item-description>(SID:s2-' . esc_html ($attr["uniqid"]) . ') First payment for immediate access.</item-description>';
												...
												$xml .= '<digital-content>';
												$xml .= '<display-disposition>OPTIMISTIC</display-disposition>';
												$xml .= '<description>You\'ll receive an email confirmation within 15 minutes.</description>';
												$xml .= '</digital-content>';
												/**/
												$xml .= '</item>';
												/**/
												...
												$xml .= '<quantity>1</quantity>';
												$xml .= '<item-name>' . esc_html ($attr["desc"]) . '</item-name>';
												$xml .= '<item-description>(SID:s2-' . esc_html ($attr["uniqid"]) . ') Cancel at any time to avoid ' . ((!$attr["rr"]) ? 'this charge' : 'charges') . '.</item-description>';
												...
												$xml .= '<quantity>1</quantity>';
												$xml .= '<item-name>' . esc_html ($attr["desc"]) . '</item-name>';
												$xml .= '<item-description>(SID:s2-' . esc_html ($attr["uniqid"]) . ') Covers ongoing access.</item-description>';
												$xml .= '<unit-price currency="' . esc_attr ($attr["cc"]) . '">' . esc_html ($attr["ra"]) . '</unit-price>';
												/**/
												$xml .= '<merchant-private-item-data>';
											...
											In other words, we should never have a Subscription w/o a Trial Period, AND no recurring charges. That would make no sense. */
											{
												...
												$xml .= '<quantity>1</quantity>';
												$xml .= '<item-name>' . esc_html ($attr["desc"]) . '</item-name>';
												$xml .= '<item-description>(SID:s2-' . esc_html ($attr["uniqid"]) . ') First payment for immediate access.</item-description>';
												$xml .= '<unit-price currency="' . esc_attr ($attr["cc"]) . '">' . esc_html ($attr["ra"]) . '</unit-price>';
												/**/
												$xml .= '<merchant-private-item-data>';
												$xml .= '<s2_subscr_id>s2-' . esc_html ($attr["uniqid"]) . '</s2_subscr_id>';
												...
												$xml .= '<quantity>1</quantity>';
												$xml .= '<item-name>' . esc_html ($attr["desc"]) . '</item-name>';
												$xml .= '<item-description>(SID:s2-' . esc_html ($attr["uniqid"]) . ') Cancel at any time to avoid charges.</item-description>';
												$xml .= '<unit-price currency="' . esc_attr ($attr["cc"]) . '">0.00</unit-price>';
												/**/
												$xml .= '<merchant-private-item-data>';
												$xml .= '<s2_subscr_id>s2-' . esc_html ($attr["uniqid"]) . '</s2_subscr_id>';
