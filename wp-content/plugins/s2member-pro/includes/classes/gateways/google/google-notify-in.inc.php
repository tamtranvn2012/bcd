						...
						if (!empty ($_GET["s2member_pro_google_notify"]) && $GLOBALS["WS_PLUGIN__"]["s2member"]["o"]["pro_google_merchant_id"])
							{
								...
								*/
								...
								/**/
								header("HTTP/1.0 200 OK"); /* Send Google® a 200 OK status header as part of the confirmation process. */
								header("Content-Type: application/xml"); /* Google® expects application/xml instead of text/xml. */
								/**/
								exit($confirmation); /* Exit w/ serial number confirmation to Google®. */
							}
					}
			}
	}
