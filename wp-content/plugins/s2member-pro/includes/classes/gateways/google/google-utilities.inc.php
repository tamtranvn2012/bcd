...
if (!class_exists ("c_ws_plugin__s2member_pro_google_utilities"))
	{
		...
		*/
		class c_ws_plugin__s2member_pro_google_utilities
			{
				/**
				* Builds an HMAC-SHA1 encrypted hash for XML data transfer verification.
				*
				* @package s2Member\Google
				* @since 1.5
				*
				* @param str $data An XML data string to sign.
				* @return str An HMAC-SHA1 hash with key encryption.
				*/
				public static function google_sign ($data = FALSE)
					{
				...
				public static function google_postvars ()
					{
						if (!empty ($_REQUEST["s2member_pro_google_notify"]) && !empty ($_REQUEST["serial-number"]))
							{
								$_request = c_ws_plugin__s2member_utils_strings::trim_deep (stripslashes_deep ($_REQUEST));
								/**/
								...
								$postback["serial-number"] = $_request["serial-number"];
								/**/
								$endpoint = ($GLOBALS["WS_PLUGIN__"]["s2member"]["o"]["pro_google_sandbox"]) ? "sandbox.google.com/checkout" : "checkout.google.com";
								/**/
								if (($response = c_ws_plugin__s2member_utils_urls::remote ("https://" . $endpoint . "/api/checkout/v2/reportsForm/Merchant/" . $GLOBALS["WS_PLUGIN__"]["s2member"]["o"]["pro_google_merchant_id"], $postback, array_merge (c_ws_plugin__s2member_pro_google_utilities::google_api_headers (), array ("timeout" => 20))))
								/**/ && wp_parse_str ($response, $postvars) !== "nill" && !empty ($postvars["_type"]))
									return $postvars; /* We can return the POST vars. */
