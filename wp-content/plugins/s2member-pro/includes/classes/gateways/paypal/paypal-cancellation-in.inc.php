														...
														if (($paypal = array ("METHOD" => "GetRecurringPaymentsProfileDetails")) && ($paypal["PROFILEID"] = $cur__subscr_id = get_user_option ("s2member_subscr_id")))
															{
																if (($paypal = c_ws_plugin__s2member_paypal_utilities::paypal_api_response ($paypal)) && empty ($paypal["__error"])) /* Have Subscription status? */
																				...
																				if (($paypal = array ("METHOD" => "ManageRecurringPaymentsProfileStatus", "ACTION" => "Cancel", "PROFILEID" => $cur__subscr_id)))
																					{
																						...
																						/**/
																						$global_response = array ("response" => '<strong>Confirmed.</strong> Your account has been cancelled. Billing terminated.');
																						/**/
																						...
																						&& wp_redirect (c_ws_plugin__s2member_pro_paypal_utilities::paypal_s2p_v_generate ($custom_success_url)) !== "nill") /* Also generate an s2p-v variable. */
																							exit ();
																					}
																			}
																		else if (preg_match ("/^(Pending|PendingProfile)$/i", $paypal["STATUS"])) /* Can't cancel while still pending. */
																			{
																				$global_response = array ("response" => 'Unable to cancel at this time. Your account is pending other changes. Please try again in 15 minutes.', "error" => true);
																			}
																		else /* Else, account already terminated. */
																			{
																				$global_response = array ("response" => 'Your account has been cancelled. Billing terminated.');
																				/**/
																				if ($post_vars["attr"]["success"] && ($custom_success_url = preg_replace ("/%%response%%/i", urlencode ($global_response["response"]), $post_vars["attr"]["success"]))/**/
																				&& wp_redirect (c_ws_plugin__s2member_pro_paypal_utilities::paypal_s2p_v_generate ($custom_success_url)) !== "nill") /* Also generate an s2p-v variable. */
																					exit ();
																	...
																	{
																		$global_response = array ("response" => 'Please <a href="https://' . (($GLOBALS["WS_PLUGIN__"]["s2member"]["o"]["paypal_sandbox"]) ? "www.sandbox.paypal.com" : "www.paypal.com") . '/cgi-bin/webscr?cmd=_subscr-find&amp;alias=' . urlencode ($GLOBALS["WS_PLUGIN__"]["s2member"]["o"]["paypal_business"]) . '" rel="nofollow">log in at PayPal®</a> to cancel your Subscription.', "error" => true);
																	}
