						...
						&& ($xco_post_vars = get_transient ("s2m_" . md5 ("s2member_transient_express_checkout_" . $_GET["token"])))/**/)/**/)
							{
								...
								/**/
								($_GET["token"]) ? delete_transient ("s2m_" . md5 ("s2member_transient_express_checkout_" . $_GET["token"])) : null;
								/**/
								if (!c_ws_plugin__s2member_pro_paypal_responses::paypal_form_attr_validation_errors ($post_vars["attr"])) /* Attr errors? */
									{
										if (!($error = c_ws_plugin__s2member_pro_paypal_responses::paypal_form_submission_validation_errors ("checkout", $post_vars)))
											{
												$cp_attr = c_ws_plugin__s2member_pro_paypal_utilities::paypal_apply_coupon ($post_vars["attr"], $post_vars["coupon"]);
														...
														if (!($paypal = array ())) /* Recurring Profile. */
															{
																...
																$paypal["SUBSCRIBERNAME"] = $full_name;
																/**/
														...
														if (($paypal = c_ws_plugin__s2member_paypal_utilities::paypal_api_response ($paypal)) && empty ($paypal["__error"]))
															{
																...
																&& ($custom_success_url = preg_replace ("/%%response%%/i", urlencode ($global_response["response"]), $ipn["s2member_paypal_proxy_return_url"])) /* Response message that would have been displayed. */
																...
																&& wp_redirect (c_ws_plugin__s2member_pro_paypal_utilities::paypal_s2p_v_generate ($custom_success_url)) !== "nill") /* Also generate an s2p-v variable; used for verification of query string vars. */
																	exit ();
															}
														else /* Else, an error. */
															{
