				...
				public static function sc_paypal_form ($attr = FALSE, $content = FALSE, $shortcode = FALSE)
					{
						...
						/**/
						c_ws_plugin__s2member_no_cache::no_cache_constants (true); /* No caching on pages that contain a Pro Form. */
						/**/
						...
						$attr["success"] = str_ireplace (array ("&#038;", "&amp;"), "&", $attr["success"]); /* Convert ampersands. */
						/**/
						$attr["accept"] = (trim ($attr["accept"])) ? preg_split ("/[;,]+/", preg_replace ("/[\r\n\t\s]+/", "", strtolower ($attr["accept"]))) : array (); /* Array. */
						$attr["accept"] = (!in_array ("paypal", $attr["accept"])) ? array_merge ($attr["accept"], array ("paypal")) : $attr["accept"]; /* Must always accept PayPal®. */
						/**/
						$attr["accept_via_paypal"] = preg_split ("/[;,]+/", preg_replace ("/[\r\n\t\s]+/", "", strtolower ($attr["accept_via_paypal"]))); /* Same as above; convert to array. */
						...
						/**/
						$attr["coupon"] = ($_GET["s2p-coupon"]) ? trim (strip_tags (stripslashes ($_GET["s2p-coupon"]))) : $attr["coupon"]; /* Allow override through query string. */
						/**/
						eval('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						...
						/**/
						/*
						Deal with cancellation forms.
						*/
						if ($attr["cancel"]) /* This handles cancellation form construction. */
							{
								$_p = c_ws_plugin__s2member_utils_strings::trim_deep (stripslashes_deep ($_POST)); /* Acquire and clean POST vars. */
								/*
								Obtain a possible response and/or validation error.
								*/
								$response = c_ws_plugin__s2member_pro_paypal_responses::paypal_cancellation_response ($attr); /* Pre-formatted response. */
								/*
								Empty post vars on successful response.
								*/
								$_p = ($response["response"] && !$response["error"]) ? array (): $_p; /* Avoids duplicate form submissions. */
								/*
								...
								*/
								$custom_template = (file_exists (TEMPLATEPATH . "/paypal-pro-cancellation-form.html")) ? TEMPLATEPATH . "/paypal-pro-cancellation-form.html" : false;
								...
								/**/
								$code = trim (file_get_contents ((($custom_template) ? $custom_template : dirname (dirname (dirname (dirname (__FILE__)))) . "/templates/forms/paypal-cancellation-form.html")));
						...
						/*
						Else if we need to deal with free registration forms.
						*/
						else if ($attr["register"]) /* This handles free registration forms. */
							{
								$_p = c_ws_plugin__s2member_utils_strings::trim_deep (stripslashes_deep ($_POST)); /* Acquire and clean POST vars. */
								/*
								Obtain a possible response and/or validation error.
								*/
								$response = c_ws_plugin__s2member_pro_paypal_responses::paypal_registration_response ($attr); /* Pre-formatted response. */
								/*
								Empty post vars on successful response.
								*/
								$_p = ($response["response"] && !$response["error"]) ? array (): $_p; /* Avoids duplicate form submissions. */
											...
											foreach (json_decode ($GLOBALS["WS_PLUGIN__"]["s2member"]["o"]["custom_reg_fields"], true) as $field)
												{
													if (in_array ($field["id"], $fields_applicable)) /* Field is applicable to Level 0? */
														{
															$tabindex = (!$tabindex) ? 90 : $tabindex; /* Start tabindex at 90. */
															/**/
															...
															$custom_fields .= c_ws_plugin__s2member_custom_reg_fields::custom_field_gen (__FUNCTION__, $field, "s2member_pro_paypal_registration[custom_fields][", "s2member-pro-paypal-registration-custom-reg-field-", "s2member-pro-paypal-custom-reg-field-" . $field_id_class . " s2member-pro-paypal-registration-custom-reg-field-" . $field_id_class, "", ($tabindex = $tabindex + 10), "", $_p, $_p["s2member_pro_paypal_registration"]["custom_fields"][$field_var], "registration");
															/**/
															$custom_fields .= '</div>' . "\n";
														}
												}
								...
								*/
								...
								Get the form template.
								*/
								$custom_template = (file_exists (TEMPLATEPATH . "/paypal-pro-registration-form.html")) ? TEMPLATEPATH . "/paypal-pro-registration-form.html" : false;
								...
								$custom_template = ($attr["template"] && file_exists (WP_CONTENT_DIR . "/" . $attr["template"])) ? WP_CONTENT_DIR . "/" . $attr["template"] : $custom_template;
								/**/
								$code = trim (file_get_contents ((($custom_template) ? $custom_template : dirname (dirname (dirname (dirname (__FILE__)))) . "/templates/forms/paypal-registration-form.html")));
								$code = (!is_multisite () || !c_ws_plugin__s2member_utils_conds::is_multisite_farm () || is_main_site ()) ? c_ws_plugin__s2member_utilities::evl ($code) : $code;
								/*
								Fill in the action.
								*/
								...
								*/
								$code = preg_replace ("/%%custom_fields%%/", c_ws_plugin__s2member_utils_strings::esc_ds ($custom_fields), $code); /* Custom Fields. */
								/*
								Fill the captcha section.
								*/
								$code = preg_replace ("/%%captcha%%/", c_ws_plugin__s2member_utils_strings::esc_ds ($captcha), $code); /* Insert a possible captcha. */
								/*
								Fill the opt-in box.
								*/
								$code = preg_replace ("/%%opt_in%%/", c_ws_plugin__s2member_utils_strings::esc_ds ($opt_in), $code); /* Insert a possible opt-in checkbox. */
								/*
								Fill hidden inputs.
								*/
								$code = preg_replace ("/%%hidden_inputs%%/", c_ws_plugin__s2member_utils_strings::esc_ds ($hidden_inputs), $code);
						...
						/*
						Else if we need to deal with billing information update forms.
						*/
						else if ($attr["update"]) /* This handles billing information updates. */
							{
								$_p = c_ws_plugin__s2member_utils_strings::trim_deep (stripslashes_deep ($_POST)); /* Acquire and clean POST vars. */
								/*
								Obtain a possible response and/or validation error.
								*/
								$response = c_ws_plugin__s2member_pro_paypal_responses::paypal_update_response ($attr); /* Pre-formatted response. */
								/*
								Empty post vars on successful response.
								*/
								$_p = ($response["response"] && !$response["error"]) ? array (): $_p; /* Avoids duplicate form submissions. */
								...
								*/
								foreach (array ("Visa" => "Visa®", "MasterCard" => "MasterCard®", "Discover" => "Discover®", "Amex" => "American Express®", "Maestro" => "Maestro®", "Solo" => "Solo®") as $card_type_v => $card_type_l)
								...
								*/
								...
								Get the form template.
								*/
								$custom_template = (file_exists (TEMPLATEPATH . "/paypal-pro-update-form.html")) ? TEMPLATEPATH . "/paypal-pro-update-form.html" : false;
								$custom_template = (file_exists (TEMPLATEPATH . "/paypal-update-form.html")) ? TEMPLATEPATH . "/paypal-update-form.html" : $custom_template;
								$custom_template = ($attr["template"] && file_exists (TEMPLATEPATH . "/" . $attr["template"])) ? TEMPLATEPATH . "/" . $attr["template"] : $custom_template;
								$custom_template = ($attr["template"] && file_exists (WP_CONTENT_DIR . "/" . $attr["template"])) ? WP_CONTENT_DIR . "/" . $attr["template"] : $custom_template;
								/**/
								$code = trim (file_get_contents ((($custom_template) ? $custom_template : dirname (dirname (dirname (dirname (__FILE__)))) . "/templates/forms/paypal-update-form.html")));
								$code = (!is_multisite () || !c_ws_plugin__s2member_utils_conds::is_multisite_farm () || is_main_site ()) ? c_ws_plugin__s2member_utilities::evl ($code) : $code;
								/*
								Fill in the action.
								*/
						...
						/*
						Else if we need to deal with Specific Post/Page forms.
						*/
						else if ($attr["sp"]) /* This handles Specific Post/Page Access. No registration required. */
							{
								$_p = c_ws_plugin__s2member_utils_strings::trim_deep (stripslashes_deep ($_POST)); /* Acquire and clean POST vars. */
								/*
								...
								*/
								...
								Obtain a possible response and/or validation error.
								*/
								$response = c_ws_plugin__s2member_pro_paypal_responses::paypal_sp_checkout_response ($attr); /* Pre-formatted response. */
								/*
								Empty post vars on successful response.
								*/
								$_p = ($response["response"] && !$response["error"]) ? array (): $_p; /* Avoids duplicate form submissions. */
								...
								Build the list of card type options.
								*/
								foreach (array ("PayPal" => "PayPal®", "Visa" => "Visa®", "MasterCard" => "MasterCard®", "Discover" => "Discover®", "Amex" => "American Express®", "Maestro" => "Maestro®", "Solo" => "Solo®") as $card_type_v => $card_type_l)
									...
									'<input aria-required="true" type="radio" name="s2member_pro_paypal_sp_checkout[card_type]" id="s2member-pro-paypal-sp-checkout-card-type-' . esc_attr (strtolower ($card_type_v)) . '" class="s2member-pro-paypal-card-type-' . esc_attr (strtolower ($card_type_v)) . ' s2member-pro-paypal-sp-checkout-card-type-' . esc_attr (strtolower ($card_type_v)) . '" value="' . ((in_array (strtolower ($card_type_v), $attr["accept_via_paypal"])) ? 'PayPal' : esc_attr ($card_type_v)) . '"' . (($_p["s2member_pro_paypal_sp_checkout"]["card_type"] === $card_type_v || (count ($attr["accept"]) === 1 && $attr["accept"][0] === "paypal" && $card_type_v === "PayPal")) ? ' checked="checked"' : '') . ((!in_array (strtolower ($card_type_v), $attr["accept"])) ? ' disabled="disabled"' : '') . ' tabindex="100" />' . "\n" .
										'</label>';
								/*
								Build the list of country code options.
								*/
								...
								*/
								...
								Get the form template.
								*/
								$custom_template = (file_exists (TEMPLATEPATH . "/paypal-pro-sp-checkout-form.html")) ? TEMPLATEPATH . "/paypal-pro-sp-checkout-form.html" : false;
								...
								$custom_template = ($attr["template"] && file_exists (WP_CONTENT_DIR . "/" . $attr["template"])) ? WP_CONTENT_DIR . "/" . $attr["template"] : $custom_template;
								/**/
								$code = trim (file_get_contents ((($custom_template) ? $custom_template : dirname (dirname (dirname (dirname (__FILE__)))) . "/templates/forms/paypal-sp-checkout-form.html")));
								$code = (count ($attr["accept"]) === 1 && $attr["accept"][0] === "paypal") ? preg_replace ("/Billing Method/", "We Accept PayPal®", $code) : $code;
								$code = (!is_multisite () || !c_ws_plugin__s2member_utils_conds::is_multisite_farm () || is_main_site ()) ? c_ws_plugin__s2member_utilities::evl ($code) : $code;
								/*
								Fill in the action.
								*/
								...
								*/
								$code = preg_replace ("/%%coupon_response%%/", c_ws_plugin__s2member_utils_strings::esc_ds (c_ws_plugin__s2member_pro_paypal_utilities::paypal_apply_coupon ($attr, $attr["coupon"], "response")), $code);
								$code = preg_replace ("/%%coupon_value%%/", c_ws_plugin__s2member_utils_strings::esc_ds ($attr["coupon"]), $code);
								/*
								Fill in the registration section.
								*/
						...
						/*
						Else deal with purchase/modification forms.
						*/
						else /* This handles initial purchase, and also modification checkout forms. */
							{
								$_p = c_ws_plugin__s2member_utils_strings::trim_deep (stripslashes_deep ($_POST)); /* Acquire and clean POST vars. */
								/*
								Configure internal attributes.
								*/
								...
								Obtain a possible response and/or validation error.
								*/
								$response = c_ws_plugin__s2member_pro_paypal_responses::paypal_checkout_response ($attr); /* Pre-formatted response. */
								/*
								Empty post vars on successful response.
								*/
								$_p = ($response["response"] && !$response["error"]) ? array (): $_p; /* Avoids duplicate form submissions. */
								...
								*/
								foreach (array ("PayPal" => "PayPal®", "Visa" => "Visa®", "MasterCard" => "MasterCard®", "Discover" => "Discover®", "Amex" => "American Express®", "Maestro" => "Maestro®", "Solo" => "Solo®") as $card_type_v => $card_type_l)
									...
									'<input aria-required="true" type="radio" name="s2member_pro_paypal_checkout[card_type]" id="s2member-pro-paypal-checkout-card-type-' . esc_attr (strtolower ($card_type_v)) . '" class="s2member-pro-paypal-card-type-' . esc_attr (strtolower ($card_type_v)) . ' s2member-pro-paypal-checkout-card-type-' . esc_attr (strtolower ($card_type_v)) . '" value="' . ((in_array (strtolower ($card_type_v), $attr["accept_via_paypal"])) ? 'PayPal' : esc_attr ($card_type_v)) . '"' . (($_p["s2member_pro_paypal_checkout"]["card_type"] === $card_type_v || (count ($attr["accept"]) === 1 && $attr["accept"][0] === "paypal" && $card_type_v === "PayPal")) ? ' checked="checked"' : '') . ((!in_array (strtolower ($card_type_v), $attr["accept"])) ? ' disabled="disabled"' : '') . ' tabindex="200" />' . "\n" .
										'</label>';
								/*
								Build the list of country code options.
								*/
													...
													if (in_array ($field["id"], $fields_applicable)) /* Field is applicable to this Level? */
														{
															...
															$custom_fields .= '<span' . ((preg_match ("/^(checkbox|pre_checkbox)$/", $field["type"])) ? ' style="display:none;"' : '') . '>' . $field["label"] . (($field["required"] === "yes") ? ' *' : '') . '</span></label>' . ((preg_match ("/^(checkbox|pre_checkbox)$/", $field["type"])) ? '' : '<br />') . "\n";
															$custom_fields .= c_ws_plugin__s2member_custom_reg_fields::custom_field_gen (__FUNCTION__, $field, "s2member_pro_paypal_checkout[custom_fields][", "s2member-pro-paypal-checkout-custom-reg-field-", "s2member-pro-paypal-custom-reg-field-" . $field_id_class . " s2member-pro-paypal-checkout-custom-reg-field-" . $field_id_class, "", ($tabindex = $tabindex + 10), "", $_p, $_p["s2member_pro_paypal_checkout"]["custom_fields"][$field_var], "registration");
															/**/
															$custom_fields .= '</div>' . "\n";
														}
												}
								...
								*/
								...
								Get the form template.
								*/
								$custom_template = (file_exists (TEMPLATEPATH . "/paypal-pro-checkout-form.html")) ? TEMPLATEPATH . "/paypal-pro-checkout-form.html" : false;
								...
								$custom_template = ($attr["template"] && file_exists (WP_CONTENT_DIR . "/" . $attr["template"])) ? WP_CONTENT_DIR . "/" . $attr["template"] : $custom_template;
								/**/
								$code = trim (file_get_contents ((($custom_template) ? $custom_template : dirname (dirname (dirname (dirname (__FILE__)))) . "/templates/forms/paypal-checkout-form.html")));
								$code = (count ($attr["accept"]) === 1 && $attr["accept"][0] === "paypal") ? preg_replace ("/Billing Method/", "We Accept PayPal®", $code) : $code;
								$code = (!is_multisite () || !c_ws_plugin__s2member_utils_conds::is_multisite_farm () || is_main_site ()) ? c_ws_plugin__s2member_utilities::evl ($code) : $code;
								/*
								Fill in the action.
