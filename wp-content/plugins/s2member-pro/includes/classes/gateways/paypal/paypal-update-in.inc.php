										...
										if (!($error = c_ws_plugin__s2member_pro_paypal_responses::paypal_form_submission_validation_errors ("update", $post_vars)))
											{
												if ($post_vars["card_type"] === "PayPal") /* A Customer must log into their PayPal® account to update billing info. */
													{
														$paypal_login = "https://" . (($GLOBALS["WS_PLUGIN__"]["s2member"]["o"]["paypal_sandbox"]) ? "www.sandbox.paypal.com" : "www.paypal.com") . "/";
														/**/
														$global_response = array ("response" => 'Please <a href="' . esc_attr ($paypal_login) . '" rel="nofollow">log in at PayPal®</a> to update your billing information.', "error" => true);
													}
												else if (is_user_logged_in () && ($user = wp_get_current_user ()) && ($user_id = $user->ID)) /* Logged in? */
													{
														if (($paypal = array ("METHOD" => "GetRecurringPaymentsProfileDetails")) && ($paypal["PROFILEID"] = $cur__subscr_id = get_user_option ("s2member_subscr_id")))
																...
																if (($paypal = c_ws_plugin__s2member_paypal_utilities::paypal_api_response ($paypal)) && empty ($paypal["__error"]) && strlen ($paypal["ACCT"]) === 4)
																	{
																		...
																		if (($paypal = c_ws_plugin__s2member_paypal_utilities::paypal_api_response ($paypal)) && empty ($paypal["__error"]))
																			{
																				$global_response = array ("response" => '<strong>Confirmed.</strong> Your billing information has been updated.');
																				/**/
																				if ($post_vars["attr"]["success"] && ($custom_success_url = preg_replace ("/%%response%%/i", urlencode ($global_response["response"]), $post_vars["attr"]["success"]))/**/
																				&& wp_redirect (c_ws_plugin__s2member_pro_paypal_utilities::paypal_s2p_v_generate ($custom_success_url)) !== "nill") /* Also generate an s2p-v variable; used for verification. */
																					exit ();
																			}
																		else /* Else, an error. */
																			{
																	...
																	{
																		$global_response = array ("response" => 'Please <a href="https://' . (($GLOBALS["WS_PLUGIN__"]["s2member"]["o"]["paypal_sandbox"]) ? "www.sandbox.paypal.com" : "www.paypal.com") . '/" rel="nofollow">log in at PayPal®</a> to update your billing information.', "error" => true);
																	}
															...
															{
																$global_response = array ("response" => 'No Subscr. ID. Please contact Support for assistance.', "error" => true);
															}
													}
												else /* Else, an error. */
													{
														$global_response = array ("response" => 'You\'re not logged in.', "error" => true);
													}
											}
										else /* Else, an error. */
											{
