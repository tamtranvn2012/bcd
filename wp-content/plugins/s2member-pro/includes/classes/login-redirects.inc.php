		...
		class c_ws_plugin__s2member_pro_login_redirects
			{
				...
				* @return bool|str A redirection URL, else the original value.
				*/
						...
						if ($redirect && ($user = $vars["user"]) && ($logins = $vars["logins"]))
							{
								foreach (preg_split ("/[\r\n\t]+/", $GLOBALS["WS_PLUGIN__"]["s2member"]["o"]["pro_login_welcome_page_otos"]) as $oto)
									{
										if (($oto = trim ($oto)) && preg_match ("/^[0-9]+\:.+$/", $oto)) /* Is the line configured properly? */
											{
												list ($number_of_logins, $url) = preg_split ("/\:/", $oto, 2);
