			...
			extends WP_Widget /* See: /wp-includes/widgets.php for further details. */
			{
				...
				public function widget ($args = FALSE, $instance = FALSE)
					{
						...
						unset ($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						if (!is_user_logged_in ()) /* If the User/Member is NOT logged in. We show the login form. */
							{
								$links = c_ws_plugin__s2member_cache::cached_page_links (); /* Cached links. */
								/**/
								$ops_page = $GLOBALS["WS_PLUGIN__"]["s2member"]["o"]["membership_options_page"];
								/**/
								$options["login_redirect"] = ($options["login_redirect"] === "%%previous%%" && $ops_page && is_page ($ops_page)) ? "" : $options["login_redirect"];
								/**/
								...
								echo '<label for="ws-plugin--s2member-pro-login-widget-username">Username:</label><br />' . "\n";
								...
								echo '<label for="ws-plugin--s2member-pro-login-widget-password">Password:</label><br />' . "\n";
								...
								echo ($options["signup_url"]) ? '<a href="' . esc_attr (($options["signup_url"] !== "%%automatic%%") ? $options["signup_url"] : $links["membership_options_page"]) . '" tabindex="-1">signup now</a> | ' : '';
								echo '<a href="' . esc_attr (wp_lostpassword_url ()) . '" tabindex="-1">forgot password?</a>' . "\n";
								echo '</div>' . "\n";
								/**/
								echo '<div class="ws-plugin--s2member-pro-login-widget-remember-me">' . "\n";
								echo '<label><input type="checkbox" name="rememberme" value="forever" />Remember Me</label>' . "\n";
								echo '</div>' . "\n";
								/**/
						...
						/**/
						else if (is_user_logged_in ()) /* We display a brief Profile Summary here. */
							{
								$user = wp_get_current_user (); /* Get current User object. */
								/**/
								$links = c_ws_plugin__s2member_cache::cached_page_links (); /* Cached links. */
								/**/
								$ops_page = $GLOBALS["WS_PLUGIN__"]["s2member"]["o"]["membership_options_page"];
								...
								echo ($options["my_profile_url"]) ? '<div class="ws-plugin--s2member-pro-login-widget-profile-summary-edit-profile"><a href="' . (($options["my_profile_url"] !== "%%automatic%%") ? esc_attr (c_ws_plugin__s2member_login_redirects::fill_login_redirect_rc_vars ($options["my_profile_url"])) : esc_attr (site_url ("/?s2member_profile=1")) . '" onclick="if(!window.open(\'' . c_ws_plugin__s2member_utils_strings::esc_sq (esc_attr (site_url ("/?s2member_profile=1"))) . '\',\'_profile\', \'width=600,height=400,left=\'+((screen.width/2)-(600/2))+\',screenX=\'+((screen.width/2)-(600/2))+\',top=\'+((screen.height/2)-(400/2))+\',screenY=\'+((screen.height/2)-(400/2))+\',location=0,menubar=0,toolbar=0,status=0,scrollbars=1,resizable=1\')) alert(\'Please disable popup blockers and try again!\'); return false;') . '">Edit My Profile</a></div>' . "\n" : '';
				...
				public function form ($instance = FALSE)
					{
						...
						*/
						...
						echo '<select class="widefat" id="' . esc_attr ($this->get_field_id ("display_gravatar")) . '" name="' . esc_attr ($this->get_field_name ("display_gravatar")) . '"><option value="1"' . (($options["display_gravatar"]) ? ' selected="selected"' : '') . '>Yes, display Gravatar</option><option value="0"' . ((!$options["display_gravatar"]) ? ' selected="selected"' : '') . '>No, do NOT display</option></select><br /><br />' . "\n";
						/**/
						echo '<label for="' . esc_attr ($this->get_field_id ("link_gravatar")) . '">Link To Gravatar.com?</label><br />' . "\n";
						echo '<select class="widefat" id="' . esc_attr ($this->get_field_id ("link_gravatar")) . '" name="' . esc_attr ($this->get_field_name ("link_gravatar")) . '"><option value="1"' . (($options["link_gravatar"]) ? ' selected="selected"' : '') . '>Yes, apply link</option><option value="0"' . ((!$options["link_gravatar"]) ? ' selected="selected"' : '') . '>No, do NOT apply</option></select><br /><br />' . "\n";
						/**/
						echo '<label for="' . esc_attr ($this->get_field_id ("display_name")) . '">Display User\'s Name?</label><br />' . "\n";
						echo '<select class="widefat" id="' . esc_attr ($this->get_field_id ("display_name")) . '" name="' . esc_attr ($this->get_field_name ("display_name")) . '"><option value="1"' . (($options["display_name"]) ? ' selected="selected"' : '') . '>Yes, display User\'s name</option><option value="0"' . ((!$options["display_name"]) ? ' selected="selected"' : '') . '>No, do NOT display</option></select><br /><br />' . "\n";
						/**/
						...
						echo '<select class="widefat" id="' . esc_attr ($this->get_field_id ("logout_redirect")) . '" name="' . esc_attr ($this->get_field_name ("logout_redirect")) . '"><option value="%%home%%"' . (($options["logout_redirect"] === "%%home%%") ? ' selected="selected"' : '') . '>Home Page</option><option value="%%previous%%"' . (($options["logout_redirect"] === "%%previous%%") ? ' selected="selected"' : '') . '>Previous page</option><option value=""' . ((!$options["logout_redirect"]) ? ' selected="selected"' : '') . '>Login screen</option></select><br /><br />' . "\n";
						/**/
						echo '<label for="' . esc_attr ($this->get_field_id ("code")) . '">Additional XHTML' . ((!is_multisite () || !c_ws_plugin__s2member_utils_conds::is_multisite_farm () || is_main_site ()) ? '/PHP' : '') . ' Code?</label><br />' . "\n";
				...
				*/
				public function configure_options_and_their_defaults ($options = FALSE)
					{
						/* Here we build the default widget options array, these get merged on a per-widget basis.
						...
						$default_options = apply_filters ("ws_plugin__s2member_pro_login_widget_default_options", array (
						/**/
						"title" => "Membership Login",/**/
						"signup_url" => "%%automatic%%",/**/
						"login_redirect" => "", /* redirect_to? */
						"logged_out_code" => "", /* 100% custom. */
						"profile_title" => "My Profile Summary",/**/
						"display_gravatar" => "1", /* Yes, display avatar. */
						"link_gravatar" => "1", /* Targer link. */
						"display_name" => "1", /* Yes, display name. */
						"logged_in_code" => "", /* 100% custom. */
						...
						foreach ($options as $key => &$value)
							{
								...
								/**/
								else if ($key === "signup_url" && !is_string ($value))
									$value = $default_options[$key];
								/**/
								else if ($key === "login_redirect" && !is_string ($value))
									$value = $default_options[$key];
								/**/
								else if ($key === "logged_out_code" && !is_string ($value))
									$value = $default_options[$key];
								/**/
								else if ($key === "profile_title" && !is_string ($value))
									$value = $default_options[$key];
								/**/
								else if ($key === "display_gravatar" && (!is_string ($value) || !is_numeric ($value)))
									$value = $default_options[$key];
								/**/
								else if ($key === "link_gravatar" && (!is_string ($value) || !is_numeric ($value)))
									$value = $default_options[$key];
								/**/
								else if ($key === "display_name" && (!is_string ($value) || !is_numeric ($value)))
									$value = $default_options[$key];
								/**/
								else if ($key === "logged_in_code" && !is_string ($value))
									$value = $default_options[$key];
								/**/
								else if ($key === "logout_redirect" && !is_string ($value))
									$value = $default_options[$key];
								/**/
								else if ($key === "my_account_url" && !is_string ($value))
									$value = $default_options[$key];
								/**/
								else if ($key === "my_profile_url" && !is_string ($value))
									$value = $default_options[$key];
							}
						/**/
