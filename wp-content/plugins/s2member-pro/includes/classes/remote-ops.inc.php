				...
				public static function remote_ops ()
					{
						if (!empty ($_GET["s2member_pro_remote_op"]) && !empty ($_POST["s2member_pro_remote_op"]))
							{
								c_ws_plugin__s2member_no_cache::no_cache_constants (true); /* No caching on Remote Operations. */
								/**/
								header("Content-Type: text/plain; charset=utf-8"); /* Responses delivered in text/plain format; with str error, or array serialization. */
								/**/
