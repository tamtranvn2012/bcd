				...
				public function __construct ()
					{
						...
						for ($n = 1; $n <= $GLOBALS["WS_PLUGIN__"]["s2member"]["c"]["levels"]; $n++)
							{
								...
								echo '<p id="ws-plugin--s2member-pro-level' . $n . '-trial-line">I\'ll offer the first <input type="text" id="ws-plugin--s2member-pro-level' . $n . '-trial-period" value="0" size="6" /> <select id="ws-plugin--s2member-pro-level' . $n . '-trial-term">' . file_get_contents (dirname (dirname (__FILE__)) . "/templates/options/ccbill-membership-trial-terms.html") . '</select> @ $<input type="text" id="ws-plugin--s2member-pro-level' . $n . '-trial-amount" value="0.00" size="4" /></p>' . "\n";
								echo '<p><span id="ws-plugin--s2member-pro-level' . $n . '-trial-then">Then, </span>I want to charge: $<input type="text" id="ws-plugin--s2member-pro-level' . $n . '-amount" value="2.95" size="4" /> / <select id="ws-plugin--s2member-pro-level' . $n . '-term">' . file_get_contents (dirname (dirname (__FILE__)) . "/templates/options/ccbill-membership-regular-terms.html") . '</select></p>' . "\n";
								...
								echo '<p>Currency: <select id="ws-plugin--s2member-pro-level' . $n . '-currency">' . file_get_contents (dirname (dirname (__FILE__)) . "/templates/options/ccbill-currencies.html") . '</select> <input type="button" value="Generate Button Code" onclick="ws_plugin__s2member_pro_ccbillButtonGenerate(\'level' . $n . '\');" class="button-primary" /></p>' . "\n";
								echo '</form>' . "\n";
								echo '</td>' . "\n";
								/**/
								echo '</tr>' . "\n";
								...
								$ws_plugin__s2member_pro_temp_s = trim (file_get_contents (dirname (dirname (__FILE__)) . "/templates/shortcodes/ccbill-checkout-button-shortcode.html"));
						...
						$ws_plugin__s2member_pro_temp_s = trim (file_get_contents (dirname (dirname (__FILE__)) . "/templates/buttons/ccbill-cancellation-button.html"));
						...
						echo '<form onsubmit="return false;">' . "\n";
						echo '<strong>WordPress® Shortcode:</strong> ( recommended for both the WordPress® Visual &amp; HTML Editors )<br />' . "\n";
						$ws_plugin__s2member_pro_temp_s = trim (file_get_contents (dirname (dirname (__FILE__)) . "/templates/shortcodes/ccbill-cancellation-button-shortcode.html"));
						...
						/**/
						echo '<p>I want to charge: $<input type="text" id="ws-plugin--s2member-pro-sp-amount" value="2.95" size="4" /> / <select id="ws-plugin--s2member-pro-sp-hours">' . file_get_contents (dirname (dirname (__FILE__)) . "/templates/options/ccbill-sp-hours.html") . '</select></p>' . "\n";
						...
						echo '<p>Currency: <select id="ws-plugin--s2member-pro-sp-currency">' . file_get_contents (dirname (dirname (__FILE__)) . "/templates/options/ccbill-currencies.html") . '</select> <input type="button" value="Generate Button Code" onclick="ws_plugin__s2member_pro_ccbillSpButtonGenerate();" class="button-primary" /></p>' . "\n";
						echo '</form>' . "\n";
						echo '</td>' . "\n";
						/**/
						echo '</tr>' . "\n";
						...
						echo '<form onsubmit="return false;">' . "\n";
						echo '<strong>WordPress® Shortcode:</strong> ( recommended for both the WordPress® Visual &amp; HTML Editors )<br />' . "\n";
						$ws_plugin__s2member_pro_temp_s = trim (file_get_contents (dirname (dirname (__FILE__)) . "/templates/shortcodes/ccbill-sp-checkout-button-shortcode.html"));
						$ws_plugin__s2member_pro_temp_s = preg_replace ("/%%custom%%/", c_ws_plugin__s2member_utils_strings::esc_ds (esc_attr ($_SERVER["HTTP_HOST"])), $ws_plugin__s2member_pro_temp_s);
						...
						echo '</optgroup></select> <a href="#" onclick="alert(\'Hold down your `Ctrl` key to select multiples.\\n\\nOptional. If you include Additional Posts/Pages, Customers will still land on your Leading Post/Page; BUT, they\\\'ll ALSO have access to some Additional Posts/Pages that you\\\'ve protected. This gives you the ability to create Post/Page Packages.\\n\\nIn other words, a Customer is sold a Specific Post/Page ( they\\\'ll land on your Leading Post/Page after checkout ), which might contain links to some other Posts/Pages that you\\\'ve packaged together under one transaction.\\n\\nBundling Additional Posts/Pages into one Package, authenticates the Customer for access to the Additional Posts/Pages automatically ( e.g. only one Access Link is needed, and s2Member generates this automatically ). However, you will STILL need to design your Leading Post/Page ( which is what a Customer will actually land on ), with links pointing to the other Posts/Pages. This way your Customers will have clickable links to everything they\\\'ve paid for.\\n\\n*Quick Summary* s2Member sends Customers to your Leading Post/Page, and also authenticates them for access to any Additional Posts/Pages automatically. You handle it from there.\\n\\n*Tip* If there are no Posts/Pages in this menu, it\\\'s because you\\\'ve not configured s2Member for Specific Post/Page Access yet. See: s2Member -> Restriction Options -> Specific Post/Page Access.\'); return false;" tabindex="-1">[?]</a></p>' . "\n";
						/**/
						echo '<p><select id="ws-plugin--s2member-pro-sp-link-hours">' . file_get_contents (dirname (dirname (__FILE__)) . "/templates/options/ccbill-sp-hours.html") . '</select> <input type="button" value="Generate Access Link" onclick="ws_plugin__s2member_pro_ccbillSpLinkGenerate();" class="button-primary" /> <img id="ws-plugin--s2member-pro-sp-link-loading" src="' . esc_attr ($GLOBALS["WS_PLUGIN__"]["s2member"]["c"]["dir_url"]) . '/images/ajax-loader.gif" alt="" style="display:none;" /></p>' . "\n";
						...
						echo '<li><code>rp="1"</code> Regular Period. Only valid w/ Membership Level Access. Must be &gt;= <code>1</code> ( ex: <code>1</code> Week, <code>2</code> Months, <code>1</code> Month, <code>3</code> Days ). ccBill® does NOT allow a Regular Billing Period to be &lt; <code>2</code> days, or &gt; <code>1</code> year. When <code>rr="1"</code> ( recurring ), ccBill® ONLY allows monthly, bi-monthly, and quarterly billing. So, for Recurring Subscriptions, you must use one of these combinations: <code>rp="1" rt="M"</code> or <code>rp="2" rt="M"</code> or <code>rp="3" rt="M"</code>.</li>' . "\n";
						echo '<li><code>rt="M"</code> Regular Term. Only valid w/ Membership Level Access. Possible values: <code>D</code> = Days, <code>W</code> = Weeks, <code>M</code> = Months, <code>Y</code> = Years. ccBill® does NOT allow a Regular Billing Period to be &lt; <code>2 days</code>, or &gt; <code>1 year</code>. When <code>rr="1"</code> ( recurring ), ccBill® ONLY allows monthly, bi-monthly, and quarterly billing. So, for Recurring Subscriptions, you must use one of these combinations: <code>rp="1" rt="M"</code> or <code>rp="2" rt="M"</code> or <code>rp="3" rt="M"</code>.</li>' . "\n";
						echo '<li><code>rr="1"</code> Recurring directive. Only valid w/ Membership Level Access. Possible values: <code>0</code> = Non-Recurring / Buy Now ( no Initial/Trial Period possible ); <code>1</code> = Recurring Subscription, with possible Initial/Trial Period at a different Trial Amount. ccBill® does NOT allow a 100% free Trial; you have to charge at least <code>2.95</code>. With ccBill®, Initial/Trial Periods are NOT possible unless <code>rr="1"</code> ( recurring ).</li>' . "\n";
