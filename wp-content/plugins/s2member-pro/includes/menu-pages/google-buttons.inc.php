				...
				public function __construct ()
					{
						...
						echo '<h2>s2Member® / Google® Checkout Buttons</h2>' . "\n";
						/**/
						...
						for ($n = 1; $n <= $GLOBALS["WS_PLUGIN__"]["s2member"]["c"]["levels"]; $n++)
							{
								echo '<div class="ws-menu-page-group" title="Google® Checkout For Level #' . $n . ' Access">' . "\n";
								/**/
								...
								echo '<p>Currency: <select id="ws-plugin--s2member-pro-level' . $n . '-currency">' . file_get_contents (dirname (dirname (__FILE__)) . "/templates/options/google-currencies.html") . '</select> <input type="button" value="Generate Button Code" onclick="ws_plugin__s2member_pro_googleButtonGenerate(\'level' . $n . '\');" class="button-primary" /></p>' . "\n";
								echo '</form>' . "\n";
								echo '</td>' . "\n";
								/**/
								echo '</tr>' . "\n";
								...
								$ws_plugin__s2member_pro_temp_s = trim (file_get_contents (dirname (dirname (__FILE__)) . "/templates/shortcodes/google-checkout-button-shortcode.html"));
								...
								echo '<strong>Resulting Google® Button Code:</strong> ( ultimately, your Shortcode will produce this snippet )<br />' . "\n";
								...
								$ws_plugin__s2member_pro_temp_s = trim (file_get_contents (dirname (dirname (__FILE__)) . "/templates/buttons/google-checkout-button.html"));
								...
								$ws_plugin__s2member_pro_temp_s = preg_replace ("/%%level%%/", c_ws_plugin__s2member_utils_strings::esc_ds (esc_attr (urlencode ($n))), $ws_plugin__s2member_pro_temp_s);
								$ws_plugin__s2member_pro_temp_s = preg_replace ("/%%ccaps%%/", c_ws_plugin__s2member_utils_strings::esc_ds (esc_attr (urlencode (""))), $ws_plugin__s2member_pro_temp_s);
								$ws_plugin__s2member_pro_temp_s = preg_replace ("/%%desc%%/", c_ws_plugin__s2member_utils_strings::esc_ds (esc_attr (urlencode ($GLOBALS["WS_PLUGIN__"]["s2member"]["o"]["level" . $n . "_label"] . " / description and pricing details."))), $ws_plugin__s2member_pro_temp_s);
								$ws_plugin__s2member_pro_temp_s = preg_replace ("/%%cc%%/", c_ws_plugin__s2member_utils_strings::esc_ds (esc_attr (urlencode ("USD"))), $ws_plugin__s2member_pro_temp_s);
								...
								$ws_plugin__s2member_pro_temp_s = preg_replace ("/%%ta%%/", c_ws_plugin__s2member_utils_strings::esc_ds (esc_attr (urlencode ("0"))), $ws_plugin__s2member_pro_temp_s);
								...
								$ws_plugin__s2member_pro_temp_s = preg_replace ("/%%ra%%/", c_ws_plugin__s2member_utils_strings::esc_ds (esc_attr (urlencode ("0.01"))), $ws_plugin__s2member_pro_temp_s);
								$ws_plugin__s2member_pro_temp_s = preg_replace ("/%%rp%%/", c_ws_plugin__s2member_utils_strings::esc_ds (esc_attr (urlencode ("1"))), $ws_plugin__s2member_pro_temp_s);
								$ws_plugin__s2member_pro_temp_s = preg_replace ("/%%rt%%/", c_ws_plugin__s2member_utils_strings::esc_ds (esc_attr (urlencode ("M"))), $ws_plugin__s2member_pro_temp_s);
								$ws_plugin__s2member_pro_temp_s = preg_replace ("/%%rr%%/", c_ws_plugin__s2member_utils_strings::esc_ds (esc_attr (urlencode ("1"))), $ws_plugin__s2member_pro_temp_s);
								$ws_plugin__s2member_pro_temp_s = preg_replace ("/%%image%%/", c_ws_plugin__s2member_utils_strings::esc_ds (esc_attr (urlencode ("default"))), $ws_plugin__s2member_pro_temp_s);
						...
						echo '<p><em><strong>*Some Hairy Details*</strong> There might be times whenever you notice that a Member\'s Subscription has been cancelled through Google® Checkout... but, s2Member continues allowing the User access to your site as a paid Member. Please don\'t be confused by this... in 99.9% of these cases, the reason for this is legitimate. s2Member will only remove the User\'s Membership privileges when an EOT ( End Of Term ) is processed, a refund occurs, a chargeback occurs, or when a cancellation occurs - which would later result in a delayed Auto-EOT by s2Member.</em></p>' . "\n";
						...
						echo '<form onsubmit="return false;">' . "\n";
						echo '<strong>WordPress® Shortcode:</strong> ( recommended for both the WordPress® Visual &amp; HTML Editors )<br />' . "\n";
						$ws_plugin__s2member_pro_temp_s = trim (file_get_contents (dirname (dirname (__FILE__)) . "/templates/shortcodes/google-cancellation-button-shortcode.html"));
						...
						echo '</select></p>' . "\n";
						echo '<p>Paid Subscr. ID: <input id="ws-plugin--s2member-pro-reg-link-subscr-id" type="text" value="" size="50" /> <a href="#" onclick="alert(\'The Customer\\\'s Paid Subscr. ID ( aka: Google® TID/SID with an `s2-` prefix ) must be unique. This value can be obtained from inside your Google® Checkout account in the transaction Description field. Each paying Customer MUST be associated with a unique Paid Subscr. ID. If the Customer is NOT associated with a Paid Subscr. ID, you will need to generate a unique value for this field on your own. But keep in mind, s2Member will be unable to maintain future communication with the Google® IPN ( i.e. Notification/Callback ) service if this value does not reflect a real Paid Subscr. ID that exists in your Google® Checkout transaction log.\'); return false;" tabindex="-1">[?]</a></p>' . "\n";
						echo '<p>Custom String Value: <input id="ws-plugin--s2member-pro-reg-link-custom" type="text" value="' . esc_attr ($_SERVER["HTTP_HOST"]) . '" size="30" /> <a href="#" onclick="alert(\'A Paid Subscription is always associated with a Custom String that is passed through the custom=\\\'\\\'' . c_ws_plugin__s2member_utils_strings::esc_sq (esc_attr ($_SERVER["HTTP_HOST"])) . '\\\'\\\' attribute of your Shortcode. This Custom Value, MUST always start with your domain name. However, you can also pipe delimit additional values after your domain, if you need to.\\n\\nFor example:\n' . c_ws_plugin__s2member_utils_strings::esc_sq (esc_attr ($_SERVER["HTTP_HOST"])) . '|cv1|cv2|cv3\'); return false;" tabindex="-1">[?]</a> <input type="button" value="Generate Access Link" onclick="ws_plugin__s2member_pro_googleRegLinkGenerate();" class="button-primary" /> <img id="ws-plugin--s2member-pro-reg-link-loading" src="' . esc_attr ($GLOBALS["WS_PLUGIN__"]["s2member"]["c"]["dir_url"]) . '/images/ajax-loader.gif" alt="" style="display:none;" /></p>' . "\n";
						...
						/**/
						echo '<p>I want to charge: $<input type="text" id="ws-plugin--s2member-pro-sp-amount" value="0.01" size="4" /> / <select id="ws-plugin--s2member-pro-sp-hours">' . file_get_contents (dirname (dirname (__FILE__)) . "/templates/options/google-sp-hours.html") . '</select></p>' . "\n";
						...
						echo '<p>Currency: <select id="ws-plugin--s2member-pro-sp-currency">' . file_get_contents (dirname (dirname (__FILE__)) . "/templates/options/google-currencies.html") . '</select> <input type="button" value="Generate Button Code" onclick="ws_plugin__s2member_pro_googleSpButtonGenerate();" class="button-primary" /></p>' . "\n";
						echo '</form>' . "\n";
						echo '</td>' . "\n";
						/**/
						echo '</tr>' . "\n";
						...
						echo '<form onsubmit="return false;">' . "\n";
						echo '<strong>WordPress® Shortcode:</strong> ( recommended for both the WordPress® Visual &amp; HTML Editors )<br />' . "\n";
						$ws_plugin__s2member_pro_temp_s = trim (file_get_contents (dirname (dirname (__FILE__)) . "/templates/shortcodes/google-sp-checkout-button-shortcode.html"));
						$ws_plugin__s2member_pro_temp_s = preg_replace ("/%%custom%%/", c_ws_plugin__s2member_utils_strings::esc_ds (esc_attr ($_SERVER["HTTP_HOST"])), $ws_plugin__s2member_pro_temp_s);
						...
						$ws_plugin__s2member_pro_temp_s = trim (file_get_contents (dirname (dirname (__FILE__)) . "/templates/buttons/google-sp-checkout-button.html"));
						$ws_plugin__s2member_pro_temp_s = preg_replace ("/%%wpurl%%/", c_ws_plugin__s2member_utils_strings::esc_ds (esc_attr (site_url ())), $ws_plugin__s2member_pro_temp_s);
						$ws_plugin__s2member_pro_temp_s = preg_replace ("/%%ids%%/", c_ws_plugin__s2member_utils_strings::esc_ds (esc_attr (urlencode ("0"))), $ws_plugin__s2member_pro_temp_s);
						$ws_plugin__s2member_pro_temp_s = preg_replace ("/%%exp%%/", c_ws_plugin__s2member_utils_strings::esc_ds (esc_attr (urlencode ("72"))), $ws_plugin__s2member_pro_temp_s);
						$ws_plugin__s2member_pro_temp_s = preg_replace ("/%%desc%%/", c_ws_plugin__s2member_utils_strings::esc_ds (esc_attr (urlencode ("Description and pricing details."))), $ws_plugin__s2member_pro_temp_s);
						$ws_plugin__s2member_pro_temp_s = preg_replace ("/%%cc%%/", c_ws_plugin__s2member_utils_strings::esc_ds (esc_attr (urlencode ("USD"))), $ws_plugin__s2member_pro_temp_s);
						$ws_plugin__s2member_pro_temp_s = preg_replace ("/%%custom%%/", c_ws_plugin__s2member_utils_strings::esc_ds (esc_attr (urlencode ($_SERVER["HTTP_HOST"]))), $ws_plugin__s2member_pro_temp_s);
						$ws_plugin__s2member_pro_temp_s = preg_replace ("/%%ra%%/", c_ws_plugin__s2member_utils_strings::esc_ds (esc_attr (urlencode ("0.01"))), $ws_plugin__s2member_pro_temp_s);
						$ws_plugin__s2member_pro_temp_s = preg_replace ("/%%sp%%/", c_ws_plugin__s2member_utils_strings::esc_ds (esc_attr (urlencode ("1"))), $ws_plugin__s2member_pro_temp_s);
						$ws_plugin__s2member_pro_temp_s = preg_replace ("/%%image%%/", c_ws_plugin__s2member_utils_strings::esc_ds (esc_attr (urlencode ("default"))), $ws_plugin__s2member_pro_temp_s);
						$ws_plugin__s2member_pro_temp_s = preg_replace ("/%%output%%/", c_ws_plugin__s2member_utils_strings::esc_ds (esc_attr (urlencode ("anchor"))), $ws_plugin__s2member_pro_temp_s);
						$ws_plugin__s2member_pro_temp_s = preg_replace ("/%%images%%/", c_ws_plugin__s2member_utils_strings::esc_ds (esc_attr ($GLOBALS["WS_PLUGIN__"]["s2member_pro"]["c"]["dir_url"] . "/images")), $ws_plugin__s2member_pro_temp_s);
						$ws_plugin__s2member_pro_temp_s = preg_replace ("/&amp;/", "&", $ws_plugin__s2member_pro_temp_s); /* Match this with the JavaScript generator. */
						...
						echo '</optgroup></select> <a href="#" onclick="alert(\'Hold down your `Ctrl` key to select multiples.\\n\\nOptional. If you include Additional Posts/Pages, Customers will still land on your Leading Post/Page; BUT, they\\\'ll ALSO have access to some Additional Posts/Pages that you\\\'ve protected. This gives you the ability to create Post/Page Packages.\\n\\nIn other words, a Customer is sold a Specific Post/Page ( they\\\'ll land on your Leading Post/Page after checkout ), which might contain links to some other Posts/Pages that you\\\'ve packaged together under one transaction.\\n\\nBundling Additional Posts/Pages into one Package, authenticates the Customer for access to the Additional Posts/Pages automatically ( e.g. only one Access Link is needed, and s2Member generates this automatically ). However, you will STILL need to design your Leading Post/Page ( which is what a Customer will actually land on ), with links pointing to the other Posts/Pages. This way your Customers will have clickable links to everything they\\\'ve paid for.\\n\\n*Quick Summary* s2Member sends Customers to your Leading Post/Page, and also authenticates them for access to any Additional Posts/Pages automatically. You handle it from there.\\n\\n*Tip* If there are no Posts/Pages in this menu, it\\\'s because you\\\'ve not configured s2Member for Specific Post/Page Access yet. See: s2Member -> Restriction Options -> Specific Post/Page Access.\'); return false;" tabindex="-1">[?]</a></p>' . "\n";
						/**/
						echo '<p><select id="ws-plugin--s2member-pro-sp-link-hours">' . file_get_contents (dirname (dirname (__FILE__)) . "/templates/options/google-sp-hours.html") . '</select> <input type="button" value="Generate Access Link" onclick="ws_plugin__s2member_pro_googleSpLinkGenerate();" class="button-primary" /> <img id="ws-plugin--s2member-pro-sp-link-loading" src="' . esc_attr ($GLOBALS["WS_PLUGIN__"]["s2member"]["c"]["dir_url"]) . '/images/ajax-loader.gif" alt="" style="display:none;" /></p>' . "\n";
