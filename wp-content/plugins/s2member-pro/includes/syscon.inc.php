...
*/
if (defined ("MEMBERSHIP_LEVELS") && is_numeric (MEMBERSHIP_LEVELS) && MEMBERSHIP_LEVELS >= $GLOBALS["WS_PLUGIN__"]["s2member"]["c"]["min_levels"] && MEMBERSHIP_LEVELS <= $GLOBALS["WS_PLUGIN__"]["s2member"]["c"]["max_levels"])
		...
		function ws_plugin__s2member_pro_options_before_checksum (&$options = array ())
			{
				...
				/*
				Backward compatibility for "pro_other_gateways_enabled", which was renamed in v1.5.
				*/
				...
				/*
				Backward compatibility for "pro_paypal_default_tax", which was renamed in v1.3.
				*/
				if (isset ($options["pro_paypal_default_tax"]))
					$options["pro_default_tax"] = $options["pro_paypal_default_tax"];
				/*
				Backward compatibility for "pro_paypal_tax_rates", which was renamed in v1.3.
				*/
						...
						if (isset ($pro_default_options[$key]) || preg_match ("/^pro_/", $key))
							{
								...
								/**/
								else if (preg_match ("/^pro_(signup|sp)_email_recipients$/", $key) && !is_string ($value)) /* Can be empty. */
									$value = $pro_default_options[$key];
								/**/
								else if (preg_match ("/^pro_(signup|sp)_email_(subject|message)$/", $key) && (!is_string ($value) || !strlen ($value)))
									$value = $pro_default_options[$key];
								/**/
								else if ($key === "pro_coupon_codes" && (!is_string ($value) || !strlen ($value)))
									$value = $pro_default_options[$key];
								/**/
