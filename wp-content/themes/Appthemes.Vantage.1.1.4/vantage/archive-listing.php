...
if ( have_posts() ) : ?>
	...
	<article class="listing">
		<h2><?php printf( __( 'Listings found for "%s" near "%s" and "%s"', APP_TD ), va_get_search_query_var( 'ls' ), va_get_search_query_var( 'location' ),va_get_search_query_var( 'cat' ) ); ?></h2>
	</article>
	<?php endif; ?>
...
<?php elseif ( !$featured ) : ?>
	<?php if ( is_search() ) : ?>
	<article class="listing">
		<h2><?php printf( __( 'Sorrys, no listings were found for "%s" near "%s" and ', APP_TD ), va_get_search_query_var( 'ls' ), va_get_search_query_var( 'location' ), va_get_search_query_var( 'cat' )); ?></h2>
	</article>
	<?php elseif ( is_archive() ) : ?>	
	<article class="listing">
		<h2><?php printf( __( 'Sorrys there are no listings for %s "%s"', APP_TD ), ( is_tax( VA_LISTING_CATEGORY ) ? __( 'category', APP_TD ) : __( 'tag', APP_TD ) ), single_term_title( '', false ) ); ?></h2>
	</article>
	<?php endif; ?>
<?php endif; ?>
	<div class="advert">
