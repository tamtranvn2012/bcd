...
$author=intval(get_query_var( 'id' ));
    ...
    $query = $wp_query->query($properties);
     ...
     if($author==$user_id||$user_id==1){
        $user_view_acc=$user_view_acc.'<div class="bntp">Edit</div><div class="bntp" >Delete</div><a href="'.get_home_url().'/create-listing/"><div class="bntp">Add product</div></a><a href="'.get_home_url().'/add-logo-sponsor/"><div class="bntp">add logoSponsor</div></a>';      
     }
?>
            <div class="authors">
...
<?php
    ...
    $author_id=$perres->post_author;
     ...
     if($author==$user_id||$user_id==1){
        $user_view_app=$user_view_app.'<div class="bntdelete" onclick="delete_product('.$pID.','.$pName.','.$author_id.','.$pUrl.')">Delete</div>';        
     }
    ...
    echo '<div class="product">';
    echo '<h2 style="font-size:18px">'.$perres->post_title.'</h2><br/>';
    echo  '<a href="#"><img src="'.wp_get_attachment_url($imagevalue).'" width="60px"></a><br/>';
    echo $user_view_app;
    echo '</div>';
    }
    ?>
